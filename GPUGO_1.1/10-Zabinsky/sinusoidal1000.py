...
'''
Python script "sinusoidal1000.py” finds the global minimum of the Sinusoidal 
function using a GPU-based global optimization method. The Sinusoidal function 
F(X) is introduced in the paper "A Numerical Evaluation of Several Stochastic 
Algorithms on Selected Continuous Global Optimization Test Problems" in 
2005 [1]. The dimension of the Sinusoidal function is set as 1000 in this 
script. In other words, the Sinusoidal function has 1000 variables, and the 
value of each variable belongs to the range [0, pi]. 
...
each subregion, and interval evaluation of the Sinusoidal function is performed 
at each sample point. The smallest upper bound for the value of the Sinusoidal 
function among all sample points is used to update the upper bound of the 
global minimum of the Sinusoidal function.
...
Notably, for the Sinusoidal function with more than 10 variables, a variable 
...
rounding in the module IASinusoidal, the result of this script has 15 or fewer 
significant digits. 

Reference:
[1] Ali, M.M., Khompatraporn, C. & Zabinsky, Z.B., (2005) "A Numerical 
Evaluation of Several Stochastic Algorithms on Selected Continuous Global 
Optimization Test Problems". J Glob Optim. 31, 635-672. 
...
'''
================================================================================
Device functions: sinusoidalValue_IA, sinusoidalSample_IA, 
sinusoidalDerivative_IA, and selectVarInSubRegion.
================================================================================
''' 

@cuda.jit(device=True)
def sinusoidalValue_IA (xValIA, pos, minUpper, flag, cycleIdx):
    
    '''
    The device function "sinusoidalValue_IA" is called by the kernel function 
    “ruleOutGPU”. The device function computes the lower and upper bounds for 
    the value of the Sinusoidal function which determines if a subregion should 
    ...
    The upper bound of the global minimum of the Sinusoidal function F(X).
    ...
    -------
    fMin: numpy.float64
    The lower bound for the value of the Sinusoidal function F(X) in the 
    subregion.
    flag: int32
    The flag equals 0 if the subregion is not ruled out. The flag equals 1 if 
    the subregion is ruled out.
    ...
    n = 1000 # The number of variables in the Sinusoidal function

    # Initialize the product of terms within the multiplication sign of 
    # sin(x_i - \pi / 6) and sin(5(x_i - \pi / 6))
    
    # F_3 = 2.5 * F_1 operation in the code list
    fMulMin, fMulMax = 2.5, 2.5
    fMul2Min, fMul2Max = 1, 1
    ...
    for i in range (1, n + 1):
        ...
        # F_m1 = x_i - \pi / 6 operation in the code list
        fMulMinCur, fMulMaxCur = ia.plus(lowerX, upperX, -math.pi / 6) 
        # F_m2 = sin (F_m1) operation in the code list
        fMul2MinCur, fMul2MaxCur = ia.sine(fMulMinCur, fMulMaxCur)    
        # F_1 = multiply (F_m2) operation in the code list
        fMulMin, fMulMax = ia.multiply(fMul2MinCur,fMul2MaxCur,fMulMin,fMulMax)   
        
        # F_m3 = 5 * F_m1 operation in the code list
        fMulMinCur, fMulMaxCur = ia.times(fMulMinCur, fMulMaxCur, 5) 
        # F_m4 = sin (F_m3) operation in the code list
        fMulMinCur, fMulMaxCur = ia.sine(fMulMinCur, fMulMaxCur)                
        # F_2 = multiply (F_m4) operation in the code list
        fMul2Min, fMul2Max=ia.multiply(fMulMinCur,fMulMaxCur,fMul2Min,fMul2Max)         

    # F_4 = F_2 + F_3 operation in the code list
    fMin, fMax = ia.add(fMulMin, fMulMax, fMul2Min, fMul2Max)   
    # F_5 = -F_4 operation in the code list
    fMin, fMax = ia.times(fMin, fMax, -1) 
...
@cuda.jit(device=True)
def sinusoidalSample_IA (xValIA, pos, num, cycleIdx):
    
    '''
    The device function "sinusoidalSample_IA" is called by the kernel function 
    “sampling”. The device function "sinusoidalSample_IA" samples over a 
    subregion and derives the smallest upper bound for the value of the 
    Sinusoidal function among all sample points in the subregion. Specifically, 
    ...
    the Sinusoidal function at each sample point are then computed using 
    interval arithmetic. The smallest upper bound for the value of the 
    Sinusoidal function among all sample points is returned as the result. 
    ...
    -------
    fMax: numpy.float64
    The smallest upper bound for the value of the Sinusoidal function among all 
    ...
    for j in range (0, num):
        ...
        for i in range (1, n + 1):
            ...
            # F_m1 = x_i - \pi / 6 operation in the code list
            fMulMinCur, fMulMaxCur = ia.plus(stepCurr, stepCurr, -math.pi / 6) 
            # F_m2 = sin (F_m1) operation in the code list
            fMul2MinCur, fMul2MaxCur = ia.sine(fMulMinCur, fMulMaxCur)    
            # F_1 = multiply (F_m2) operation in the code list
            fMulMin, fMulMax = ia.multiply(fMul2MinCur,fMul2MaxCur,
                fMulMin,fMulMax)   
            ...
            fMulMinCur, fMulMaxCur = ia.sine(fMulMinCur, fMulMaxCur)                
            # F_2 = multiply (F_m4) operation in the code list
            fMul2Min, fMul2Max=ia.multiply(fMulMinCur,fMulMaxCur,
                fMul2Min,fMul2Max) 
        ...
        # value of the Sinusoidal function among all sample points within the 
        # subregion
        if (j == 0):
            ...
            # the value of the Sinusoidal function among all sample points in 
            # the subregion
            if (fMaxOut > fMax):
                fMaxOut = fMax
...
@cuda.jit(device=True)
def sinusoidalDerivative_IA (xdIA, pos, flagVal, cycleIdx):
    
    '''
    Device function "sinusoidalDerivative_IA" is called by the kernel function 
    ...
    # F_3 = 2.5 * F_1 operation in the code list
    dfMulMin, dfMulMax = 2.5, 2.5
    # F_4 = 5 * F_2 operation in the code list
    dfMul2Min, dfMul2Max = 5, 5
    ...
    for i in range (1, n + 1):
        ...
        # F_m1 = x_i - \pi / 6 operation in the code list
        dfMulMinCur, dfMulMaxCur = ia.plus(lowerX, upperX, -math.pi / 6) 
        # F_m2 = sin (F_m1) operation in the code list
        dfMul2MinCur, dfMul2MaxCur = ia.sine(dfMulMinCur, dfMulMaxCur)    
        # F_1 = multiply (F_m2) operation in the code list
        dfMulMin,dfMulMax=ia.multiply(dfMul2MinCur,dfMul2MaxCur,
            dfMulMin,dfMulMax)   
        
        # F_m3 = 5 * F_m1 operation in the code list
        dfMulMinCur, dfMulMaxCur = ia.times(dfMulMinCur, dfMulMaxCur, 5) 
        # F_m4 = sin (F_m3) operation in the code list
        dfMulMinCur, dfMulMaxCur = ia.sine(dfMulMinCur, dfMulMaxCur)                
        # F_2 = multiply (F_m4) operation in the code list
        dfMul2Min, dfMul2Max=ia.multiply(dfMulMinCur,dfMulMaxCur,
            dfMul2Min,dfMul2Max)      
    
    # Calculate the first order derivatives of the Sinusoidal Function            
    for j in range (1, n + 1):
        lowerX, upperX = selectVarInSubRegion (xdIA, j, pos, cycleIdx)  
        # F_5 = x_j - \pi / 6 operation in the code list
        df5Min, df5Max = ia.plus(lowerX, upperX, -math.pi / 6)     
        # F_6 = sin (F_2) operation in the code list
        dfMin, dfMax = ia.sine(df5Min, df5Max)  
        
        # F_10 = 5 * F_5 operation in the code list
        df10Min, df10Max = ia.times(df5Min, df5Max, 5)   
        # F_11 = sin (F_5) operation in the code list
        df11Min, df11Max = ia.sine(df10Min, df10Max)   
        ...
        if (dfMin == -np.inf or dfMax == np.inf or 
            df11Min == -np.inf or df11Max == np.inf):
            flag = 0
        else:   
            # F_7 = F_3 / F_6 operation in the code list
            dfMin, dfMax = ia.divide(dfMulMin, dfMulMax, dfMin, dfMax)     
            # F_8 = cos (F_2) operation in the code list
            df8Min, df8Max = ia.cosine(df5Min, df5Max)   
            # F_9 = F_7 * F_8 operation in the code list
            dfMin, dfMax = ia.multiply(dfMin, dfMax, df8Min, df8Max)     
            
            # F_12 = F_4 / F_11 operation in the code list
            df11Min, df11Max = ia.divide(dfMul2Min, dfMul2Max, df11Min, df11Max)     
            # F_13 = cos (F_10) operation in the code list
            df8Min, df8Max = ia.cosine(df10Min, df10Max)   
            # F_14 = F_12 * F_13 operation in the code list
            df11Min, df11Max = ia.multiply(df11Min, df11Max, df8Min, df8Max)   
            
            # F_15 = F_9 + F_14 operation in the code list
            dfMin, dfMax = ia.add(dfMin, dfMax, df11Min, df11Max)   
            # F_16 = -F_15 operation in the code list
            dfMin, dfMax = ia.times(dfMin, dfMax, -1)     

            # Check the derivative to determine if the subregion could be ruled 
            # out
            if (dfMin > 0 and lowerX != 0):
                flag = 1
                return flag
            elif (dfMax < 0 and upperX != math.pi):
                flag = 1
                return flag
...
def selectVarInSubRegion (xVal, num, pos, cycleIdx):
    ...
    "sinusoidalValue_IA", "sinusoidalSample_IA", and "sinusoidalDerivative_IA". 
    ...
    length = 1000 # The number of variables in the Sinusoidal function
...
def sampling (region, output_lowestUpper, numSamples, cycIdx):
    ...
    computes the smallest upper bound for the value of the Sinusoidal function 
    among all sample points in the selected region to update the upper bound of 
    the global minimum of the Sinusoidal function. Specifically, each thread in 
    the same block computes the smallest upper bound for the value of the 
    Sinusoidal function among all sample points in its corresponding subregion 
    ...
    global minimum of the Sinusoidal function (stored on GPU global memory) 
    ...
    The upper bound of the global minimum of the Sinusoidal function. The value 
    is initialized by the upper bound of the global minimum of the Sinusoidal 
    ...
    value of the Sinusoidal function derived by threads in the same block. The 
    ...
    # Compute the smallest upper bound for the value of the Sinusoidal function 
    # among all sample points in a subregion 
    shr[thr] = sinusoidalSample_IA (region, pos, numSamples[0], cycIdx[0])
    ...
    minimum of the Sinusoidal function using atomic operations.
    '''
    if thr == 0:
...
def ruleOutGPU (region, result, atomic_index, lowestUpper, lowerBounds, cycIdx):
    ...
    could exist and the lower bounds for the value of the Sinusoidal function in 
    ...
    "sinusoidalValue_IA" which rules out suboptimal subregions through computing 
    the lower bound for the value of the function. Then, the device function 
    "sinusoidalDerivative_IA" is called to further rule out suboptimal 
    ...
    the global minimum of the Sinusoidal function F(X). This array is 
    ...
    The array stores the lower bounds for the value of the Sinusoidal function 
    ...
    fMin, flag = sinusoidalValue_IA (region,pos,lowestUpper[0],flag,cycIdx[0])
    # NOTE: Comment OUT the line if do NOT use derivative rule-out
    flag = sinusoidalDerivative_IA (region, pos, flag, cycIdx[0])
    if flag == 0:
...
def completeRegion (xVal, pos, cycleIdx):
    ...
    # the value of all variables X_i in the subregion in the Sinusoidal function
...
# the 1000 variables in the Sinusoidal function. 

initialRegion = np.tile([0, math.pi], 1000)
...
# Initialize the maximum cycling index for the Sinusoidal function with 1000 
# variables
maximumCyc = 991
...
# to each region that may include the global minimum of the Sinusoidal function)
...
# the global minimum of the Sinusoidal function F(X)
widthTolerance = 1e-4
...
# Print the number of variables for the Sinusoidal function, width tolerance and 
# the number of samples for the sampling process
print("\n ---------------------------------------------------------------")
print("Number of variables for the Sinusoidal function:", 1000)
...
while (terminate == False):
    ...
    # and update the upper bound of the global minimum of the Sinusoidal 
    # function using the smallest upper bound for the value of the 
    # Sinusoidal function among all sample points in the selected region
    ...
    # Transfer the upper bound of the global minimum of the Sinusoidal 
    ...
    # Update the upper bound of the global minimum of the Sinusoidal 
    # function 
    ...
    # possibly containing the global minimum of the Sinusoidal function F(X))
    ...
    # value of the Sinusoidal function in the subregions that could contain the 
    # global minimum 
    ...
    # bound of the global minimum of the Sinusoidal function F(X) computed from 
    ...
    # the values of the Sinusoidal function from GPU to CPU
    ...
    # contain the global minimum of the Sinusoidal function by appending the 
    # corresponding information of the subregions that are not ruled out (lower 
    # bounds for the value of the Sinusoidal function in the current iteration, 
    ...
    # The region with the smallest lower bound for the value of the Sinusoidal 
    ...
    if (lengthList != 0):
        while (found == False):
            ...
            # Obtain the lower bound for the value of the Sinusoidal function in 
            # the selected region
            ...
            # Note: For the Sinusoidal function with MORE THAN 10 variables, the 
