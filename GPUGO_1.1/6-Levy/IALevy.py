...
'''
Python script "IALevy.py” implements the basic interval arithmetic operations 
on GPU-based devices for interval evaluation of the Levy function [1] and its 
...
arithmetic operation returns results with 15 significant digits. The outwardly 
rounded interval contains all possible results for an interval arithmetic 
operation.
...
def ru(n, decimals=0):
    ...
    '''
    Function "ru" rounds a number up to a specified number of decimal places.
...
def rd(n, decimals=0):
    ...
    '''
    Function "rd" rounds a number down to a specified number of decimal places.
...
def outRound(lower, upper, n):
    ...
    '''
    Function "outRound" outwardly rounds the bounds of an interval to the 
...
def power(lower, upper, pow):
    ...
    '''
    Function "power" implements the interval arithmetic operation of raising
    ...
    if lower < 0 and upper > 0 and pow % 2 == 0:
