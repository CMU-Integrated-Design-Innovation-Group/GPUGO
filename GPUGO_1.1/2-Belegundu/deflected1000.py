...
'''
Python script "deflected1000.py” finds the global minimum of the Deflected 
Corrugated Spring function using a GPU-based global optimization method. The 
Deflected Corrugated Spring function F(X) is first introduced in the work 
Optimization Concepts and Applications in Engineering in 2011 [1]. The 
dimension of the Deflected Corrugated Spring function is set as 1000 in this 
script. In other words, the Deflected Corrugated Spring function has 1000 
variables, and the value of each variable belongs to the range [-10, 11]. 
...
each subregion, and interval evaluation of the Deflected Corrugated Spring 
function is performed at each sample point. The smallest upper bound for the 
value of the Deflected Corrugated Spring function among all sample points is 
used to update the upper bound of the global minimum of the Deflected Corrugated 
Spring function.
...
Notably, for the Deflected Corrugated Spring function with more than 10 
variables, a variable cycling technique is employed. Specifically, when the 
...
rounding in the module IADeflected, the result of this script has 15 or fewer 
significant digits. 

Reference:
[1] Belegundu, A., & Chandrupatla, T., 2011 Optimization Concepts and 
Applications in Engineering. Cambridge University Press. 
https://doi.org/10.1017/CBO9780511975905

'''

from numba import cuda
import numpy as np
import IADeflected as ia
import math
import time
...
'''
================================================================================
Device functions: deflectedValue_IA, deflectedSample_IA, deflectedDerivative_IA, 
and selectVarInSubRegion.
================================================================================
''' 

@cuda.jit(device=True)
def deflectedValue_IA (xValIA, pos, minUpper, flag, cycleIdx):
    
    '''
    The device function "deflectedValue_IA" is called by the kernel function 
    “ruleOutGPU”. The device function computes the lower and upper bounds for 
    the value of the Deflected Corrugated Spring function which determines if a 
    subregion should be ruled out using a code list. A code list defines the 
    sequence of interval arithmetic operations to evaluate the function. The 
    bounds for variables within the subregion in the function F(X) are 
    calculated based on the absolute position of the thread in the grid in GPU 
    computing and the cycling index of the selected region. After the evaluation 
    process using the code list, the computed lower bound for the value of the 
    function that belongs to the subregion is compared with the smallest upper 
    bound for the global minimum to decide if the subregion should be ruled out 
    from the space. Specifically, if the lower bound is smaller than or equal 
    to the smallest upper bound, the subregion should be kept with the flag 
    value of 0. If the lower bound is larger than the smallest upper bound, the 
    ...
    The upper bound of the global minimum of the Deflected Corrugated Spring 
    function F(X).
    ...
    -------
    fMin: numpy.float64
    The lower bound for the value of the Deflected Corrugated Spring function 
    F(X) in the subregion.
    ...
    # The number of variables in the Deflected Corrugated Spring function
    n = 1000 

    # Interval evaluation of the Deflected Corrugated Spring function in the 
    # subregion
    
    fMin, fMax = 0, 0 
    
    # Initialize the sum of terms within the summation sign of (x_i - 5)^2
    
    fsMin, fsMax = 0, 0

    # Compute the sum of terms within the summation sign of (x_i - 5)^2
    for i in range (1, n + 1):
        ...
        # F_s1 = x_i - 5 operation in the code list
        fsMinCurr, fsMaxCurr = ia.plus(lowerX, upperX, -5) 
        # F_s2 = (F_s1) ^ 2 operation in the code list
        fsMinCurr, fsMaxCurr = ia.power(fsMinCurr, fsMaxCurr, 2)    
        # F_1 = sum (F_s2) operation in the code list
        fsMin, fsMax = ia.add(fsMinCurr, fsMaxCurr, fsMin, fsMax)            

    # F_2 = 0.1 * F_1 operation in the code list
    fMin, fMax = ia.times(fsMin, fsMax, 0.1)   
    # F_3 = (F_1) ^ 0.5 operation in the code list
    f3Min, f3Max = ia.power(fsMin, fsMax, 0.5) 
    # F_4 = 5 F_3 operation in the code list
    f3Min, f3Max = ia.times(f3Min, f3Max, 5) 
    # F_5 = cos (F_4) operation in the code list
    f3Min, f3Max = ia.cosine(f3Min, f3Max) 
    # F_6 = F_2 - F_5 operation in the code list  
...
@cuda.jit(device=True)
def deflectedSample_IA (xValIA, pos, num, cycleIdx):
    
    '''
    The device function "deflectedSample_IA" is called by the kernel function 
    “sampling”. The device function "deflectedSample_IA" samples over a 
    subregion and derives the smallest upper bound for the value of the 
    Deflected Corrugated Spring function among all sample points in the 
    subregion. Specifically, the bounds for the value of each variable within 
    the subregion (the position of the subregion) are first calculated using 
    the absolute position of the thread in the grid that corresponds to the 
    ...
    for the value of the Deflected Corrugated Spring function at each sample 
    point are then computed using interval arithmetic. The smallest upper bound 
    for the value of the Deflected Corrugated Spring function among all sample 
    points is returned as the result. 
    ...
    -------
    fMax: numpy.float64
    The smallest upper bound for the value of the Deflected Corrugated Spring 
    function among all sample points in the subregion.
    ...
    for j in range (0, num):
        ...
        for i in range (1, n + 1):
            ...
            # F_s1 = x_i - 5 operation in the code list
            fsMinCurr, fsMaxCurr = ia.plus(stepCurr, stepCurr, -5) 
            # F_s2 = (F_s1) ^ 2 operation in the code list
            fsMinCurr, fsMaxCurr = ia.power(fsMinCurr, fsMaxCurr, 2)    
            # F_1 = sum (F_s2) operation in the code list
            fsMin, fsMax = ia.add(fsMinCurr, fsMaxCurr, fsMin, fsMax)            
        ...
        # value of the Deflected Corrugated Spring function among all sample 
        # points within the subregion
        if (j == 0):
            # Initialize the value of fMaxOut in the first iteration
            fMaxOut = fMax
        else:
            # Update the value of fMaxOut to derive the smallest upper bound for 
            # the value of the Deflected Corrugated Spring function among all 
...
@cuda.jit(device=True)
def deflectedDerivative_IA (xdIA, pos, flagVal, cycleIdx):
    
    '''
    Device function "deflectedDerivative_IA" is called by the kernel function 
    ...
    # Calculate the first order derivatives of the Deflected Corrugated Spring 
    # Function            
    for j in range (1, n + 1):
        lowerX, upperX = selectVarInSubRegion (xdIA, j, pos, cycleIdx)
        
        # F_3 = 5 F_2 operation in the code list
        dfMin, dfMax = ia.times(dtempMin, dtempMax, 5)     
        # F_4 = sin (F_3) operation in the code list
        dfMin, dfMax = ia.sine(dfMin, dfMax)    
        # F_5 = 5 F_4 operation in the code list
        dfMin, dfMax = ia.times(dfMin, dfMax, 5)  
        # F_6 = x_j - 5 operation in the code list
        df6Min, df6Max = ia.plus(lowerX, upperX, -5)
        # F_7 = F_5 * F_6 operation in the code list
        dfMin, dfMax = ia.multiply(dfMin, dfMax, df6Min, df6Max)      
        # F_8 = F_7 / F_2 operation in the code list
        dfMin, dfMax = ia.divide(dfMin, dfMax, dtempMin, dtempMax)  
        # F_9 = 0.2 F_6 operation in the code list
        df6Min, df6Max = ia.times(df6Min, df6Max, 0.2)  
        # F_10 = F_9 + F_8 operation in the code list
        dfMin, dfMax = ia.add(df6Min, df6Max, dfMin, dfMax)       
...
def selectVarInSubRegion (xVal, num, pos, cycleIdx):
    ...
    "deflectedValue_IA", "deflectedSample_IA", and "deflectedDerivative_IA". 
...
def sampling (region, output_lowestUpper, numSamples, cycIdx):
    ...
    computes the smallest upper bound for the value of the Deflected Corrugated 
    Spring function among all sample points in the selected region to update 
    the upper bound of the global minimum of the Deflected Corrugated Spring 
    function. Specifically, each thread in the same block computes the smallest 
    upper bound for the value of the Deflected Corrugated Spring function among 
    all sample points in its corresponding subregion and stores the smallest 
    upper bound on an array on GPU shared memory. The smallest element on the 
    array is derived through a reduction tree. The smallest element on the 
    array is used to update the upper bound of the global minimum of the 
    Deflected Corrugated Spring function (stored on GPU global memory) using 
    atomic operations. 
    ...
    The upper bound of the global minimum of the Deflected Corrugated Spring 
    function. The value is initialized by the upper bound of the global minimum 
    of the Deflected Corrugated Spring function derived from the last iteration.  
    ...
    value of the Deflected Corrugated Spring function derived by threads in the 
    same block. The size of the array on GPU shared memory needs to be 
    consistent with the number of threads per block in GPU computing.
    '''
    shr = cuda.shared.array(512, dtype = np.float64)

    # Compute the smallest upper bound for the value of the Deflected 
    # Corrugated Spring function among all sample points in a subregion 
    shr[thr] = deflectedSample_IA (region, pos, numSamples[0], cycIdx[0])
    ...
    minimum of the Deflected Corrugated Spring function using atomic operations.
    '''
    if thr == 0:
...
def ruleOutGPU (region, result, atomic_index, lowestUpper, lowerBounds, cycIdx):
    ...
    could exist and the lower bounds for the value of the Deflected Corrugated 
    Spring function in these remaining subregions. The index of each subregion 
    is represented by the absolute position of the thread in the grid in GPU 
    computing. For the rule-out process, the kernel function first calls the 
    device function "deflectedValue_IA" which rules out suboptimal subregions 
    through computing the lower bound for the value of the function. Then, the 
    device function "deflectedDerivative_IA" is called to further rule out 
    ...
    the global minimum of the Deflected Corrugated Spring function F(X). This 
    ...
    The array stores the lower bounds for the value of the Deflected Corrugated 
    Spring function in the subregions that are not ruled out.
    ...
    fMin, flag = deflectedValue_IA (region,pos,lowestUpper[0],flag,cycIdx[0])
    # NOTE: Comment OUT the line if do NOT use derivative rule-out
    flag = deflectedDerivative_IA (region, pos, flag, cycIdx[0])
    if flag == 0:
...
def completeRegion (xVal, pos, cycleIdx):
    ...
    # the value of all variables X_i in the subregion in the Deflected 
...
# the 1000 variables in the Deflected Corrugated Spring function. 
...
# Initialize the maximum cycling index for the Deflected Corrugated Spring 
# function with 1000 variables
maximumCyc = 991
...
# to each region that may include the global minimum of the Deflected 
# Corrugated Spring function)
...
# the global minimum of the Deflected Corrugated Spring function F(X)
widthTolerance = 1e-4
...
# Print the number of variables for the Deflected Corrugated Spring function, 
# width tolerance and the number of samples for the sampling process
print("\n ---------------------------------------------------------------")
print("Number of variables for the Deflected Corrugated Spring function:", 1000)
...
while (terminate == False):
    ...
    # and update the upper bound of the global minimum of the Deflected 
    # Corrugated Spring function using the smallest upper bound for the 
    # value of the Deflected Corrugated Spring function among all sample 
    ...
    # Transfer the upper bound of the global minimum of the Deflected 
    # Corrugated Spring function onto GPU global memory
    ...
    # the Deflected Corrugated Spring function 
    ...
    # Update the upper bound of the global minimum of the Deflected 
    # Corrugated Spring function 
    ...
    # possibly containing the global minimum of the Deflected Corrugated Spring 
    # function F(X))
    ...
    # value of the Deflected Corrugated Spring function in the subregions that 
    # could contain the global minimum 
    ...
    # bound of the global minimum of the Deflected Corrugated Spring function 
    # F(X) computed from the FIRST kernel function.
    ...
    # the values of the Deflected Corrugated Spring function from GPU to CPU
    ...
    # contain the global minimum of the Deflected Corrugated Spring function by 
    # appending the corresponding information of the subregions that are not 
    # ruled out (lower bounds for the value of the Deflected Corrugated Spring 
    # function in the current iteration, the index of the current iteration, 
    # the cycling index of the selected region, and the absolute positions of 
    # threads in the grid) after the CPU or GPU rule-out process.
    ...
    # The region with the smallest lower bound for the value of the Deflected 
    # Corrugated Spring function among all regions in the region list is 
    ...
    if (lengthList != 0):
        while (found == False):
            ...
            # Obtain the lower bound for the value of the Deflected Corrugated 
            # Spring function in the selected region
            ...
            # Note: For the Deflected Corrugated Spring function with MORE THAN 
            # 10 variables, the width for each variable should be the same ONLY 
            # when the cycling index of the selected region at the current 
            # iteration equals the maximum cycling index.
