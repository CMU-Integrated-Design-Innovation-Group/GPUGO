...
'''
Python script "IACorrupted.py” implements the basic interval arithmetic 
operations on GPU-based devices for interval evaluation of the Multimodal 
Corrupted Convex Quadratic Function [1].
...
def outRound(lower, upper, n):
    ...
    if lower == 0:
        ...
        # NOTE: The following condition is added to avoid the case where the 
        # absolute value of lower is smaller than 1 * (10 ** (-294)). In such 
        # cases, the lower bound may cause the outward rounding to be incorrect 
        # with overflow error. Thus, the lower bound is multiplied by 100 to 
        # avoid the overflow error (outwardly rounded to a larger number).
        if (-lower < 1 * (10 ** (-294))):
           lower = lower * 100
           
        lowerOut = -ru(-lower, n - 1 - math.floor(math.log10(-lower)))
    else:
        # NOTE: The following condition is added to avoid the case where lower 
        # is smaller than 1 * (10 ** (-294)). In such cases, the lower bound
        # may cause the outward rounding to be incorrect with overflow error.
        # Thus, the lower bound is outwardly rounded to 0 to avoid the overflow 
        # error.
        if (lower < 1 * (10 ** (-294))):
            lowerOut = 0
        else:
    ...
    if upper == 0:
        ...
        # NOTE: The following condition is added to avoid the case where the 
        # absolute value of upper is smaller than 1 * (10 ** (-294)). In such 
        # cases, the upper bound may cause the outward rounding to be incorrect 
        # with overflow error. Thus, the upper bound is outwardly rounded to 0 
        # to avoid the overflow error.
        if (-upper < 1 * (10 ** (-294))):
            upperOut = 0
        else:
            upperOut = -rd(-upper, n - 1 - math.floor(math.log10(-upper)))
    else:
        # NOTE: The following condition is added to avoid the case where upper 
        # is smaller than 1 * (10 ** (-294)). In such cases, the upper bound
        # may cause the outward rounding to be incorrect with overflow error.
        # Thus, the upper bound is multiplied by 100 to avoid the overflow 
        # error (outwardly rounded to a larger number).
        if (upper < 1 * (10 ** (-294))):
           upper = upper * 100
