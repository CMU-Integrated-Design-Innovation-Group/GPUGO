...
'''
Python script "rosenbrock1000.py” finds the global minimum of the Rosenbrock
Function using a GPU-based global optimization method. The Rosenbrock function 
is first introduced in the work “An Automatic Method for Finding the Greatest 
or Least Value of a Function” by H. H. Rosenbrock in 1960 [1]. The 
n-dimensional form of the function is formally presented by Y. W. Shang and 
Y. H. Qiu [2]. The dimension of the Rosenbrock Function is set as 1000 in 
this script. In other words, the Rosenbrock Function has 1000 variables, and 
the value of each variable belongs to the range [-30, 30]. 
...
each subregion, and interval evaluation of the Rosenbrock Function is performed 
at each sample point. The smallest upper bound for the value of the Rosenbrock 
...
Notably, for the Rosenbrock Function with more than 
10 variables, a variable cycling technique is employed. Specifically, when the 
selected region is partitioned into many subregions in Step 2 and Step 3, only 
10 dimensions of the selected region is partitioned in turn at each iteration, 
instead of partitioning all its dimensions at the same time. 

When the iterative process is halted, the 1000-dimensional region(s) that 
...
rounding in the module IARosenbrock, the result of this script has 15 or fewer 
significant digits. 

Reference:
[1] Rosenbrock, H. H. An Automatic Method for Finding the Greatest or Least 
Value of a Function. Comput. J. 3(3), 175-184 (1960).
https://doi.org/10.1093/comjnl/3.3.175
[2] Shang, Y. W. and Qiu, Y. H. A Note on the Extended Rosenbrock Function. 
Evol. Comput. 14(1), 119-126 (2006).
https://doi.org/10.1162/106365606776022733

'''

from numba import cuda
import numpy as np
import IARosenbrock as ia
import math
import time
...
'''
================================================================================
Device functions: rosenbrockValue_IA, rosenbrockSample_IA, 
rosenbrockDerivative_IA, and selectVarInSubRegion.
================================================================================
''' 

@cuda.jit(device=True)
def rosenbrockValue_IA (xValIA, pos, minUpper, flag, cycleIdx):
    
    '''
    The device function "rosenbrockValue_IA" is called by the kernel function 
    “ruleOutGPU”. The device function computes the lower and upper bounds for 
    the value of the Rosenbrock Function which 
    determines if a subregion should be ruled out using a code list. A code 
    list defines the sequence of interval arithmetic operations to evaluate the 
    function. The bounds for variables within the subregion in the function 
    F(X) are calculated based on the absolute position of the thread in the 
    grid in GPU computing and the cycling index of the selected region. After 
    the evaluation process using the code list, the computed lower bound for 
    the value of the function that belongs to the subregion is compared with 
    the smallest upper bound for the global minimum to decide if the subregion 
    should be ruled out from the space. Specifically, if the lower bound is 
    smaller than or equal to the smallest upper bound, the subregion should be 
    kept with the flag value of 0. If the lower bound is larger than the 
    smallest upper bound, the subregion should be ruled out with the flag 
    value of 1. 
    ...
    in the function F(X), where i = 1, 2, … n. In this script, n = 1000.

    pos: int32
    The absolute position of a thread in the grid in GPU computing.

    minUpper: numpy.float64
    The upper bound of the global minimum of the Rosenbrock Function F(X).
    ...
    -------
    fMin: numpy.float64
    The lower bound for the value of the Rosenbrock Function F(X) in the 
    subregion.
    ...
    '''
    # The number of variables in the Rosenbrock Function
    n = 1000 
    ...
    for i in range (1, n):
        ...
        # F_s1 = x_i - 1 operation in the code list
        fsMinCur, fsMaxCur = ia.plus(lowerX, upperX, -1) 
        # F_s2 = F_s1 ^ 2 operation in the code list
        fsMinCur, fsMaxCur = ia.power(fsMinCur, fsMaxCur, 2) 

        # F_s3 = (x_i) ^ 2 operation in the code list
        fsMinCurTemp, fsMaxCurTemp = ia.power(lowerX, upperX, 2) 
        # Update the lower and upper bounds for the values of variables X_{i+1} 
        # in each iteration
        lowerX, upperX=selectVarInSubRegion (xValIA, i+1, pos, cycleIdx)
        # F_s4 = F_s3 - x_{i+1} operation in the code list
        fsMinCurTemp, fsMaxCurTemp = ia.minus(fsMinCurTemp, fsMaxCurTemp,
            lowerX, upperX) 
        # F_s5 = (F_s4) ^ 2 operation in the code list
        fsMinCurTemp, fsMaxCurTemp = ia.power(fsMinCurTemp, fsMaxCurTemp, 2) 
        # F_s6 = 100 * F_s5 operation in the code list
        fsMinCurTemp, fsMaxCurTemp = ia.times(fsMinCurTemp, fsMaxCurTemp, 100) 
        # F_s7 = F_s6 + F_s2 operation in the code list
        fsMinCur, fsMaxCur = ia.add(fsMinCurTemp, fsMaxCurTemp, 
            fsMinCur, fsMaxCur) 
        # F_1 = sum (F_s7) operation in the code list
        fMin, fMax = ia.add(fMin, fMax, fsMinCur, fsMaxCur)   
...
@cuda.jit(device=True)
def rosenbrockSample_IA (xValIA, pos, num, cycleIdx):
    
    '''
    The device function "rosenbrockSample_IA" is called by the kernel function 
    “sampling”. The device function "rosenbrockSample_IA" samples over a 
    subregion and derives the smallest upper bound for the value of the 
    Rosenbrock Function among all sample points in 
    the subregion. Specifically, the bounds for the value of each variable 
    within the subregion (the position of the subregion) are first calculated 
    using the absolute position of the thread in the grid that corresponds to 
    the subregion, the bounds for the value of each variable within the 
    selected region (the position of the selected region), and the cycling 
    index of the selected region. User-specified number of sample points are 
    uniformly chosen along the diagonal of the subregion, where the diagonal 
    connects the two vertices of the subregion with the coordinates of lower 
    bounds and upper bounds for all variables, respectively. The lower and 
    upper bounds for the value of the Rosenbrock Function at each sample point 
    are then computed using interval arithmetic. The smallest upper bound for 
    the value of the Rosenbrock Function among all sample points is returned as 
    the result. 
    ...
    function F(X), where i belongs to {1, 2, …, n}. In this script, n = 1000.
    ...
    -------
    fMax: numpy.float64
    The smallest upper bound for the value of the Rosenbrock Function among all 
    ...
    for j in range (0, num):
        ...
        for i in range (1, n):
            ...
            # F_s1 = x_i - 1 operation in the code list
            fsMinCur, fsMaxCur = ia.plus(stepCurr, stepCurr, -1) 
            # F_s2 = F_s1 ^ 2 operation in the code list
            fsMinCur, fsMaxCur = ia.power(fsMinCur, fsMaxCur, 2) 

            # F_s3 = (x_i) ^ 2 operation in the code list
            fsMinCurTemp, fsMaxCurTemp = ia.power(stepCurr, stepCurr, 2) 
            # Update the lower and upper bounds for the values of variables 
        ...
        # value of the Rosenbrock Function among all 
        # sample points within the subregion
        if (j == 0):
            ...
            # among all sample points in the subregion
            if (fMaxOut > fMax):
                fMaxOut = fMax
...
@cuda.jit(device=True)
def rosenbrockDerivative_IA (xdIA, pos, flagVal, cycleIdx):
    
    '''
    Device function "rosenbrockDerivative_IA" is called by the kernel function 
    ...
    # Calculate the first order derivatives of the Rosenbrock Function
    for i in range (1, n + 1):

        if i == 1:
            # x_1
            lowerX, upperX = selectVarInSubRegion (xdIA, i, pos, cycleIdx)  
            # F_1 = x_1 - 1 operation in the code list
            dfMin, dfMax = ia.plus(lowerX, upperX, -1)  
            # F_2 = 2 * F_1 operation in the code list
            dfMin, dfMax = ia.times(dfMin, dfMax, 2)  
            # F_3 = x_1 ^ 2 operation in the code list
            dfMinTemp, dfMaxTemp = ia.power(lowerX, upperX, 2)  
            # x_2
            lowerXTemp, upperXTemp = selectVarInSubRegion (xdIA, i+1, 
                pos, cycleIdx)  
            # F_4 = F_3 - x_2 operation in the code list
            dfMinTemp, dfMaxTemp = ia.minus(dfMinTemp, dfMaxTemp, 
                lowerXTemp,upperXTemp)  
            # F_5 = F_4 * x_1 operation in the code list
            dfMinTemp, dfMaxTemp = ia.multiply(dfMinTemp, dfMaxTemp, 
                lowerX, upperX)  
            # F_6 = 400 * F_5 operation in the code list
            dfMinTemp, dfMaxTemp = ia.times(dfMinTemp, dfMaxTemp, 400)  
            # F_7 = F_2 + F_6 operation in the code list
            dfMin, dfMax = ia.add(dfMin, dfMax, dfMinTemp, dfMaxTemp)  

        elif i == n:
            # x_{n-1}
            lowerX, upperX = selectVarInSubRegion (xdIA, i-1, pos, cycleIdx)  
            # F_17 = x_{n-1} ^ 2 operation in the code list
            dfMin, dfMax = ia.power(lowerX, upperX, 2)  
            # x_n
            lowerX, upperX = selectVarInSubRegion (xdIA, i, pos, cycleIdx)  
            # F_18 = x_n - F_17 operation in the code list
            dfMin, dfMax = ia.minus(lowerX, upperX, dfMin, dfMax)  
            # F_19 = 200 * F_18 operation in the code list
            dfMin, dfMax = ia.times(dfMin, dfMax, 200)  
        
        else:
            # x_i
            lowerX, upperX = selectVarInSubRegion (xdIA, i, pos, cycleIdx)  
            # F_8 = x_i ^ 2 operation in the code list
            dfMin, dfMax = ia.power(lowerX, upperX, 2)  
            # x_{i+1}
            lowerXTemp, upperXTemp = selectVarInSubRegion (xdIA, i+1, 
                pos, cycleIdx)  
            # F_9 = F_8 - x_{i+1} operation in the code list
            dfMin, dfMax = ia.minus(dfMin, dfMax, lowerXTemp, upperXTemp)  
            # F_10 = 400 * F_9 operation in the code list
            dfMin, dfMax = ia.times(dfMin, dfMax, 400)  
            # F_11 = F_10 + 202 operation in the code list
            dfMin, dfMax = ia.plus(dfMin, dfMax, 202)  
            # F_12 = F_11 * x_i operation in the code list
            dfMin, dfMax = ia.multiply(dfMin, dfMax, lowerX, upperX)  
            # F_13 = F_12 - 2 operation in the code list
            dfMin, dfMax = ia.plus(dfMin, dfMax, -2)  
            # x_{i-1}
            lowerXTemp, upperXTemp = selectVarInSubRegion (xdIA, i-1, 
                pos, cycleIdx)  
            # F_14 = x_{i-1} ^ 2 operation in the code list
            dfMinTemp, dfMaxTemp = ia.power(lowerXTemp, upperXTemp, 2)  
            # F_15 = 200 * F_14 operation in the code list
            dfMinTemp, dfMaxTemp = ia.times(dfMinTemp, dfMaxTemp, 200)  
            # F_16 = F_13 - F_15 operation in the code list
            dfMin, dfMax = ia.minus(dfMin, dfMax, dfMinTemp, dfMaxTemp)  

        # Check the derivative to determine if the subregion could be ruled 
        # out
        if (dfMin > 0 and lowerX != -30):
            flag = 1
            return flag
        elif (dfMax < 0 and upperX != 30):
            flag = 1
            return flag
...
def selectVarInSubRegion (xVal, num, pos, cycleIdx):
    ...
    "rosenbrockValue_IA", "rosenbrockSample_IA", and "rosenbrockDerivative_IA". 
...
def sampling (region, output_lowestUpper, numSamples, cycIdx):
    ...
    computes the smallest upper bound for the value of the Rosenbrock Function 
    among all sample points in the selected region to update the upper bound of 
    the global minimum of the Rosenbrock Function. Specifically, each thread in 
    the same block computes the smallest upper bound for the value of the 
    Rosenbrock Function among all sample points in its corresponding subregion 
    ...
    global minimum of the Rosenbrock Function (stored on GPU global memory) 
    ...
    The upper bound of the global minimum of the Rosenbrock Function. The value 
    is initialized by the upper bound of the global minimum of the Rosenbrock 
    ...
    value of the Rosenbrock Function derived by 
    threads in the same block. The size of the array on GPU shared memory needs 
    to be consistent with the number of threads per block in GPU computing.
    '''
    shr = cuda.shared.array(512, dtype = np.float64)

    # Compute the smallest upper bound for the value of the Rosenbrock Function 
    # among all sample points in a subregion 
    shr[thr] = rosenbrockSample_IA (region, pos, numSamples[0], cycIdx[0])
    ...
    minimum of the Rosenbrock Function using atomic 
    operations.
    '''
    if thr == 0:
...
def ruleOutGPU (region, result, atomic_index, lowestUpper, lowerBounds, cycIdx):
    ...
    could exist and the lower bounds for the value of the Rosenbrock Function 
    in these remaining subregions. The index of each subregion is represented 
    by the absolute position of the thread in the grid in GPU computing. For 
    the rule-out process, the kernel function first calls the device function 
    "rosenbrockValue_IA" which rules out suboptimal subregions through computing 
    the lower bound for the value of the function. Then, the device function 
    "rosenbrockDerivative_IA" is called to further rule out suboptimal 
    subregions through computing the first order derivatives of the function. 
    Specifically, the flag equals 0 if the subregion is not ruled out. The flag 
    equals 1 if the subregion is ruled out. 
    ...
    F(X). This array is initialized before the execution of the kernel function. 
    ...
    The array stores the lower bounds for the value of the Rosenbrock Function 
    in the subregions that are not ruled out.
    ...
    fMin, flag = rosenbrockValue_IA (region,pos,lowestUpper[0],flag,cycIdx[0])
    # NOTE: Comment OUT the line if do NOT use derivative rule-out
    flag = rosenbrockDerivative_IA (region, pos, flag, cycIdx[0])
    if flag == 0:
...
'''
================================================================================
Host function: completeRegion, combine, connectivity.
================================================================================
'''
...
def completeRegion (xVal, pos, cycleIdx):
    ...
    # the value of all variables X_i in the subregion in the Rosenbrock Function
...
def combine (outputSingle, outputCombined, solutionTolerance):

    '''
    Host function "combine" combines or adds the newly obtained output 
    region(root) into the result list("outputCombined" array). By checking the 
    connectivity of the existing regions in the result list with the newly 
    obtained region, the newly obtained region is either being added to the 
    "outputCombined" array as a new root of the problem, or is combined with 
    existing root(s) in the "outputCombined" array. Specifically, the combining
    process takes the smallest lower bound and the largest upper bound of the
    existing root and the output region for each of the variables in the region.

    Parameters
    ----------
    outputSingle: A 1-D NumPy array (numpy.float64) represents the output
    region that is pending to be added to the result list or combined with 
    exisitng root in the result list.

    outputCombined: A 2-D NumPy array (numpy.float64) represents the result 
    regions that includes the root(s) of the problem.

    solutionTolerance: float64 
    The solution tolerance for combining the output regions with existing 
    results in the "outputCombined" array.
    ...
    length = len(outputCombined)
    # If the result array is empty, add the output region into the list
    if (length == 0):
        outputCombined += [outputSingle] 
        return None
    else:
        # Iterate through all elements of the result array. If the output region
        # and any element(region) in the result array are considered as 
        # connected, the output region is combined with the element(region)
        # in the result array.
        for i in range (0, length):
            root = outputCombined[i]
            # Check for connectivity and combine the regions
            if (connectivity (root, outputSingle, solutionTolerance)):
                rootReshaped = root.reshape(-1, 2)
                outputSingleReshaped = outputSingle.reshape(-1, 2)
                lowerUnion = np.minimum(rootReshaped[:, 0], 
                    outputSingleReshaped[:, 0])
                upperUnion = np.maximum(rootReshaped[:, 1], 
                    outputSingleReshaped[:, 1])   
                union = np.empty_like(outputSingle)
                union[0::2] = lowerUnion
                union[1::2] = upperUnion
                outputCombined[i] = union
                return None
        # If the output region cannot be combined, it is added as a new root
        # to the result array.
        outputCombined += [outputSingle]     
        return None

def connectivity (root, new, solutionTolerance): 

    '''
    Host function "connectivity" checks if the newly obtained region(root) from 
    the pipeline should be combined with an existing region(root) in the result 
    array. There are two cases in which the regions will be combined. First,
    if the absolute differences between the upper bounds of the existing 
    region(root) in the result array and the lower bounds of the newly obtained 
    region are all smaller than the solution tolerance for all variables, the 
    regions should be combined. Second, if the absolute differences between the 
    lower bounds of the existing region(root) in the result array and the upper 
    bounds of the newly obtained region are all smaller than the solution 
    tolerance for all variables, the regions should also be combined. 
    Notably, as a special case, if the existing region(root) in the result 
    array fully contains the newly obtained region, the combine process is not 
    necessary.

    Parameters
    ----------
    root: A 1-D NumPy array (numpy.float64) represents the existing root in the 
    result list.

    new: A 1-D NumPy array (numpy.float64) represents the newly obtained
    region that is pending to be added to the result list or combined with 
    existing root in the result list.
    ...
    -------
    combine: Boolean
    The flag to determine if the regions(roots) should be combined. If combine
    is True, the regions should be combined. If combine is False, the regions
    ...
    for i in range (0, len(root) // 2):
        rootCurrLower, rootCurrUpper = root[2 * i], root[2 * i + 1]
        newCurrLower, newCurrUpper = new[2 * i], new[2 * i + 1]
        # Take the two absolute differences
        diffLower = abs(newCurrLower - rootCurrUpper)
        diffUpper = abs (newCurrUpper - newCurrLower)
        # If the existing root fully contains the newly obtained region,
        # the combine process is not necessay.
        if ((rootCurrLower <= newCurrLower and newCurrLower <= rootCurrUpper) 
            and
            (rootCurrLower <= newCurrUpper and newCurrUpper <= rootCurrUpper)):
            pass
        else:
            if (diffLower < solutionTolerance):
                pass
            else:
                return combine
            if (diffUpper < solutionTolerance):
                pass
            else:
                return combine
    combine = True
...
# Define a NumPy array that represents the 1000-dimensional search domain. 
# The NumPy array is initialized by the lower and upper bounds for the value of 
# the 1000 variables in the Rosenbrock Function. 

initialRegion = np.tile([-30, 30], 1000)
...
# Initialize the maximum cycling index for the Rosenbrock Function with 1000 
# variables
maximumCyc = 991
...
# to each region that may include the global minimum of the Rosenbrock Function)
cycling_list = np.array([], dtype = np.int32)

# Initialize the initial smallest upper bound for the global minimum as 
# 1000000000
lowestUpperVal = 1000000000
...
# the global minimum of the Rosenbrock Function F(X)
widthTolerance = 1e-4

# Define the tolerance for combining the solutions
combineTolerance = 1e-3
...
# Print the number of variables for the Rosenbrock Function, width tolerance 
# and the number of samples for the sampling process
print("\n ---------------------------------------------------------------")
print("Number of variables for the Rosenbrock Function:", 1000)
...
while (terminate == False):
    ...
    # LowestUpper is initialized as 1000000000 for the first iteration.
    ...
    # and update the upper bound of the global minimum of the Rosenbrock 
    # Function using the smallest upper bound for the value of the Rosenbrock 
    ...
    # Transfer the upper bound of the global minimum of the Rosenbrock Function 
    ...
    # Update the upper bound of the global minimum of the Rosenbrock Function
    ...
    # possibly containing the global minimum of the Rosenbrock Function F(X))
    ...
    # value of the Rosenbrock Function in the 
    # subregions that could contain the global minimum 
    ...
    # bound of the global minimum of the Rosenbrock Function F(X) computed from 
    ...
    # the values of the Rosenbrock Function from GPU 
    # to CPU
    ...
    # contain the global minimum of the Rosenbrock Function by appending the 
    # corresponding information of the subregions that are not ruled out (lower 
    # bounds for the value of the Rosenbrock Function in the current iteration, 
    ...
    # The region with the smallest lower bound for the value of the Rosenbrock 
    ...
    # global minimum. If the width is not smaller than the width tolerance, 
    # the region is selected as the region to be partitioned and analyzed 
    # in the next iteration step. The iterative process terminates when 
    # one of the two cases occurs: (1) the region selected for the next 
    # iteration step is found; (2) the region is added to the output array, and 
    # there is no region remained in the region list to be processed.
    ...
    if (lengthList != 0):
        while (found == False):
            ...
            # Obtain the lower bound for the value of the Rosenbrock Function 
            ...
            if (iter == 0 and lowestUpperVal != 1000000000):
            ...
            # Note: For the Rosenbrock Function with 
            # MORE THAN 10 variables, the width for each variable should be the 
            # same ONLY when the cycling index of the selected region at the 
            # current iteration equals the maximum cycling index.
            if (width < widthTolerance and cycleIdxVal == maximumCyc):
                # Add the region to the output array
                output += [selected_region]
                outputValLower += [lowerB]

                # Combine the region to the final result
                combine (selected_region, combinedOutput, combineTolerance)
...
# Print the combined region
print ("Combined regions that may include the global minimum are:", 
     combinedOutput)

print ("Length of the Combined regions array:", 
     len(combinedOutput))
