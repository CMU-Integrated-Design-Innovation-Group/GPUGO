...
'''
Python script "cosine1000.py” finds the global minimum of the Cosine Mixture 
function using a GPU-based global optimization method. The Cosine Mixture 
function F(X) is adapted from the work “A Deterministic Algorithm for Global 
Optimization” in 1993 [1]. The dimension of the Cosine Mixture function is set 
as 1000 in this script. In other words, the Cosine Mixture function has 1000 
variables, and the value of each variable belongs to the range [-1, 2]. 
...
each subregion, and interval evaluation of the Cosine Mixture function is 
performed at each sample point. The smallest upper bound for the value of the 
Cosine Mixture function among all sample points is used to update the upper 
bound of the global minimum of the Cosine Mixture function.
...
Notably, for the Cosine Mixture function with more than 10 variables, a variable 
...
the user-specified width tolerance (region(s) in the result array), where the 
global minimum of the objective function could exist, and the lower and upper 
bounds for the value of the global minimum of the objective function are 
outputted. 
...
rounding in the module IACosine, the result of this script has 15 or fewer 
significant digits. 

Reference:
[1] Breiman, L., & Cutler, A. (1993) "A Deterministic Algorithm for Global 
Optimization." Math. Program. 58(1-3), 179-199.
https://doi.org/10.1007/BF01581266

'''

from numba import cuda
import numpy as np
import IACosine as ia
import math
import time
...
'''
================================================================================
Device functions: cosineValue_IA, cosineSample_IA, cosineDerivative_IA, 
and selectVarInSubRegion.
================================================================================
''' 

@cuda.jit(device=True)
def cosineValue_IA (xValIA, pos, minUpper, flag, cycleIdx):
    
    '''
    The device function "cosineValue_IA" is called by the kernel function 
    “ruleOutGPU”. The device function computes the lower and upper bounds for 
    the value of the Cosine Mixture function which determines if a subregion 
    ...
    cycling index of the selected region. After the evaluation process using the 
    code list, the computed lower bound for the value of the function that 
    ...
    smallest upper bound, the subregion should be kept with the flag value of 0. 
    If the lower bound is larger than the smallest upper bound, the subregion 
    should be ruled out with the flag value of 1. 
    ...
    The upper bound of the global minimum of the Cosine Mixture function F(X).
    ...
    -------
    fMin: numpy.float64
    The lower bound for the value of the Cosine Mixture function F(X) in the 
    subregion.
    flag: int32
    The flag equals 0 if the subregion is not ruled out. The flag equals 1 if 
    the subregion is ruled out.
    ...
    n = 1000 # The number of variables in the Cosine Mixture function

    # Initialize the sum of terms within the summation sign of cos(5 \pi x_i)
    fsMin, fsMax = 0, 0 
    # Initialize the sum of terms within the summation sign of (x_i) ^ 2
    fs2Min, fs2Max = 0, 0 

    # Compute the sum of terms within the summation sign of cos(5 \pi x_i)
    for i in range (1, n + 1):
        ...
        # F_s1 = 5 \pi x_i operation in the code list
        fsMinCur, fsMaxCur = ia.times(lowerX, upperX, 5 * math.pi)
        # F_s2 = cos (F_s1) operation in the code list
        fsMinCur, fsMaxCur = ia.cosine(fsMinCur, fsMaxCur)
        ...
        # F_3 = sum (F_s3) operation in the code list
        fs2Min, fs2Max = ia.add(fs2Min, fs2Max, fsMinCur, fsMaxCur)  
    # F_2 = -0.1 * F_1 operation in the code list
    fMin, fMax = ia.times(fsMin, fsMax, -0.1)        

    # F_4 = F_2 + F_3 operation in the code list
    fMin, fMax = ia.add(fMin, fMax, fs2Min, fs2Max)       
...
@cuda.jit(device=True)
def cosineSample_IA (xValIA, pos, num, cycleIdx):
    
    '''
    The device function "cosineSample_IA" is called by the kernel function 
    “sampling”. The device function "cosineSample_IA" samples over a 
    subregion and derives the smallest upper bound for the value of the 
    Cosine Mixture function among all sample points in the subregion. 
    ...
    selected region. User-specified number of sample points are uniformly chosen 
    along the diagonal of the subregion, where the diagonal connects the two 
    vertices of the subregion with the coordinates of lower bounds and upper 
    bounds for all variables, respectively. The lower and upper bounds for the 
    value of the Cosine Mixture function at each sample point are then computed 
    using interval arithmetic. The smallest upper bound for the value of the 
    Cosine Mixture function among all sample points is returned as the result. 
    ...
    -------
    fMax: numpy.float64
    The smallest upper bound for the value of the Cosine Mixture function among 
    ...
    for j in range (0, num):
        # Initialize the sum of terms within the summation 
        # sign of cos(5 \pi x_i)
        fsMin, fsMax = 0, 0 
        # Initialize the sum of terms within the summation sign of (x_i) ^ 2
        fs2Min, fs2Max = 0, 0 
        ...
        for i in range (1, n + 1):
            ...
            # F_s1 = 5 \pi x_i operation in the code list
            fsMinCur, fsMaxCur = ia.times(stepCurr, stepCurr, 5 * math.pi)
            # F_s2 = cos (F_s1) operation in the code list
            fsMinCur, fsMaxCur = ia.cosine(fsMinCur, fsMaxCur)
            # F_1 = sum (F_s2) operation in the code list
            fsMin, fsMax = ia.add(fsMin, fsMax, fsMinCur, fsMaxCur)  
        ...
        # value of the Cosine Mixture function among all sample points within 
        # the subregion
        if (j == 0):
            ...
            # the value of the Cosine Mixture function among all sample points 
            # in the subregion
            if (fMaxOut > fMax):
                fMaxOut = fMax
...
@cuda.jit(device=True)
def cosineDerivative_IA (xdIA, pos, flagVal, cycleIdx):
    
    '''
    Device function "cosineDerivative_IA" is called by the kernel function 
    ...
    # Calculate the first order derivatives of the Cosine Mixture Function            
    for j in range (1, n + 1):
        ...
        dfMin, dfMax = 0, 0
        # F_1 = 5 * \pi x_i operation in the code list
        dfMin, dfMax = ia.times(lowerX, upperX, 5 * math.pi) 
        # F_2 = sin (F_1) operation in the code list
        dfMin, dfMax = ia.sine(dfMin, dfMax)    
        # F_3 = 0.5 \pi F_3 operation in the code list
        dfMin, dfMax = ia.times(dfMin, dfMax, 0.5 * math.pi)  
        # F_4 = 2 * x_i operation in the code list
        df4Min, df4Max = ia.times(lowerX, upperX, 2) 
        # F_5 = F_3 + F_4 operation in the code list
        dfMin, dfMax = ia.add(dfMin, dfMax, df4Min, df4Max)   

        # Check the derivative to determine if the subregion could be ruled out
        if (dfMin > 0 and lowerX != -1):
            flag = 1
            return flag
        elif (dfMax < 0 and upperX != 2):
            flag = 1
            return flag
...
def selectVarInSubRegion (xVal, num, pos, cycleIdx):
    ...
    "cosineValue_IA", "cosineSample_IA", and "cosineDerivative_IA". 
    ...
    length = 1000 # The number of variables in the Cosine Mixture function
...
def sampling (region, output_lowestUpper, numSamples, cycIdx):
    ...
    computes the smallest upper bound for the value of the Cosine Mixture 
    function among all sample points in the selected region to update the upper 
    bound of the global minimum of the Cosine Mixture function. Specifically, 
    each thread in the same block computes the smallest upper bound for the 
    value of the Cosine Mixture function among all sample points in its 
    ...
    upper bound of the global minimum of the Cosine Mixture function (stored on 
    ...
    The upper bound of the global minimum of the Cosine Mixture function. The 
    value is initialized by the upper bound of the global minimum of the Cosine 
    Mixture function derived from the last iteration.  
    ...
    value of the Cosine Mixture function derived by threads in the same block. 
    ...
    # Compute the smallest upper bound for the value of the Cosine Mixture 
    # function among all sample points in a subregion 
    shr[thr] = cosineSample_IA (region, pos, numSamples[0], cycIdx[0])
    ...
    minimum of the Cosine Mixture function using atomic operations.
    '''
    if thr == 0:
...
def ruleOutGPU (region, result, atomic_index, lowestUpper, lowerBounds, cycIdx):
    ...
    could exist and the lower bounds for the value of the Cosine Mixture 
    function in these remaining subregions. The index of each subregion is 
    represented by the absolute position of the thread in the grid in GPU 
    computing. For the rule-out process, the kernel function first calls the 
    device function "cosineValue_IA" which rules out suboptimal subregions 
    through computing the lower bound for the value of the function. Then, the 
    device function "cosineDerivative_IA" is called to further rule out 
    ...
    the global minimum of the Cosine Mixture function F(X). This array is 
    ...
    The array stores the lower bounds for the value of the Cosine Mixture 
    ...
    fMin, flag = cosineValue_IA (region,pos,lowestUpper[0],flag,cycIdx[0])
    # NOTE: Comment OUT the line if do NOT use derivative rule-out
    flag = cosineDerivative_IA (region, pos, flag, cycIdx[0])
    if flag == 0:
...
def completeRegion (xVal, pos, cycleIdx):
    ...
    # the value of all variables X_i in the subregion in the Cosine Mixture 
    # function
...
# the 1000 variables in the Cosine Mixture function. 

initialRegion = np.tile([-1, 2], 1000)
...
# Initialize the maximum cycling index for the Cosine Mixture function with 1000 
# variables
maximumCyc = 991
...
# to each region that may include the global minimum of the Cosine Mixture 
# function)
...
# the global minimum of the Cosine Mixture function F(X)
widthTolerance = 1e-4
...
# Print the number of variables for the Cosine Mixture function, width tolerance 
# and the number of samples for the sampling process
print("\n ---------------------------------------------------------------")
print("Number of variables for the Cosine Mixture function:", 1000)
...
while (terminate == False):
    ...
    # and update the upper bound of the global minimum of the Cosine Mixture 
    # function using the smallest upper bound for the value of the Cosine 
    # Mixture function among all sample points in the selected region
    ...
    # Transfer the upper bound of the global minimum of the Cosine Mixture 
    ...
    # Update the upper bound of the global minimum of the Cosine Mixture 
    # function 
    lowestUpperVal = lowestUpper_host[0]
    
    # DEBUGGING STATEMENT:
    # print ("The GPU-Based Sampling Result:", lowestUpperVal)    
    ...
    # possibly containing the global minimum of the Cosine Mixture function 
    # F(X))
    ...
    # value of the Cosine Mixture function in the subregions that could contain 
    # the global minimum 
    ...
    # bound of the global minimum of the Cosine Mixture function F(X) computed 
    ...
    # the values of the Cosine Mixture function from GPU to CPU
    ...
    # contain the global minimum of the Cosine Mixture function by appending the 
    # corresponding information of the subregions that are not ruled out (lower 
    # bounds for the value of the Cosine Mixture function in the current 
    ...
    # The region with the smallest lower bound for the value of the Cosine 
    # Mixture function among all regions in the region list is selected and then 
    ...
    if (lengthList != 0):
        while (found == False):
            ...
            # Obtain the lower bound for the value of the Cosine Mixture 
            # function in the selected region
            ...
            # Note: For the Cosine Mixture function with MORE THAN 10 variables, 
