...
'''
Python script "corrupted1000.py” finds the global minimum of the Multimodal 
Corrupted Convex Quadratic function using a GPU-based global optimization 
method. The Multimodal Corrupted Convex Quadratic function F(X) is introduced 
in the work “Experiments in Nonconvex Optimization: Stochastic Approximation 
with Function Smoothing and Simulated Annealing” [1] in 1990. The dimension of 
the Multimodal Corrupted Convex Quadratic function is set as 1000 in this 
script. In other words, the Multimodal Corrupted Convex Quadratic function has 
1000 variables, and the value of each variable belongs to the range [-10, 11]. 
...
each subregion, and interval evaluation of the Multimodal Corrupted Convex 
Quadratic function is performed at each sample point. The smallest upper bound 
for the value of the Multimodal Corrupted Convex Quadratic function among all 
sample points is used to update the upper bound of the global minimum of the 
Multimodal Corrupted Convex Quadratic function. 
...
Notably, for the Multimodal Corrupted Convex Quadratic function with more than 
...
rounding in the module IACorrupted, the result of this script has 15 or fewer 
significant digits. 

Reference:
[1] M. Styblinski and T. Tang, (1990) “Experiments in Nonconvex Optimization: 
Stochastic Approximation with Function Smoothing and Simulated Annealing,” 
Neuron Networks, 3, 467-483.
https://doi.org/10.1016/0893-6080(90)90029-K

'''

from numba import cuda
import numpy as np
import IACorrupted as ia
import math
import time
...
'''
================================================================================
Device functions: corruptedValue_IA, corruptedSample_IA, corruptedDerivative_IA, 
and selectVarInSubRegion.
================================================================================
''' 

@cuda.jit(device=True)
def corruptedValue_IA (xValIA, pos, minUpper, flag, cycleIdx):
    
    '''
    The device function "corruptedValue_IA" is called by the kernel function 
    “ruleOutGPU”. The device function computes the lower and upper bounds for 
    the value of the Multimodal Corrupted Convex Quadratic function which 
    ...
    The upper bound of the global minimum of the Multimodal Corrupted Convex 
    ...
    -------
    fMin: numpy.float64
    The lower bound for the value of the Multimodal Corrupted Convex Quadratic 
    function F(X) in the subregion.
    ...
    '''
    # The number of variables in the Multimodal Corrupted Convex Quadratic 
    # function
    n = 1000 

    # Initialize the product of terms within the multiplication sign of cos(x_i)
    # F_3 = -4n * F_1 operation in the code list
    fMin, fMax = -4*n, -4*n

    # Compute the product of terms within the multiplication sign of cos(x_i)
    for i in range (1, n + 1):
        ...
        # F_m1 = cosine (x_i) operation in the code list
        fsMinCur, fsMaxCur = ia.cosine(lowerX, upperX)  
        # F_1 = multiply(F_m1) operation in the code list
        fMin, fMax = ia.multiply(fMin, fMax, fsMinCur, fsMaxCur)    
    ...
    for i in range (1, n + 1):
        ...
        # F_2 = sum (F_s1) operation in the code list
        fsMin, fsMax = ia.add(fsMin, fsMax, fsMinCur, fsMaxCur)   
    # F_4 = (1/2n) * F_2 operation in the code list
    fsMin, fsMax = ia.times(fsMin, fsMax, 1 / (2 * n))
    # F_5 = F_1 + F_4 operation in the code list
...
@cuda.jit(device=True)
def corruptedSample_IA (xValIA, pos, num, cycleIdx):
    
    '''
    The device function "corruptedSample_IA" is called by the kernel function 
    “sampling”. The device function "corruptedSample_IA" samples over a 
    subregion and derives the smallest upper bound for the value of the 
    Multimodal Corrupted Convex Quadratic function among all sample points in 
    ...
    upper bounds for the value of the Multimodal Corrupted Convex Quadratic 
    function at each sample point are then computed using interval arithmetic. 
    The smallest upper bound for the value of the Multimodal Corrupted Convex 
    Quadratic function among all sample points is returned as the result. 
    ...
    fMax: numpy.float64
    The smallest upper bound for the value of the Multimodal Corrupted Convex 
    Quadratic function among all sample points in the subregion.
    ...
    for j in range (0, num):
        ...
        # Initialize the product of terms within the multiplication sign of 
        # cos(x_i)
        # F_3 = -4n * F_1 operation in the code list
        fMin, fMax = -4*n, -4*n

        # Compute the product of terms within the multiplication sign of 
        # cos(x_i)
        for i in range (1, n + 1):
            ...
            # F_m1 = cosine (x_i) operation in the code list
            fsMinCur, fsMaxCur = ia.cosine(stepCurr, stepCurr)  
            # F_1 = multiply(F_m1) operation in the code list
            fMin, fMax = ia.multiply(fMin, fMax, fsMinCur, fsMaxCur)    
        ...
        # value of the Multimodal Corrupted Convex Quadratic function among all 
        ...
        if (j == 0):
            ...
            # the value of the Multimodal Corrupted Convex Quadratic function 
...
@cuda.jit(device=True)
def corruptedDerivative_IA (xdIA, pos, flagVal, cycleIdx):
    
    '''
    Device function "corruptedDerivative_IA" is called by the kernel function 
    ...
    for i in range (1, n + 1):
        ...
        # F_m1 = cosine (x_i) operation in the code list
        fmMinCur, fmMaxCur = ia.cosine(lowerX, upperX)  
        # F_1 = multiply(F_m1) operation in the code list
        fMulMin, fMulMax = ia.multiply(fMulMin, fMulMax, fmMinCur, fmMaxCur)  
    
    # Calculate the first order derivatives of the Multimodal Corrupted Convex 
    # Quadratic Function            
    for j in range (1, n + 1):
        lowerX, upperX = selectVarInSubRegion (xdIA, j, pos, cycleIdx)  
        
        # F_2 = cos (x_j) operation in the code list
        dfMin, dfMax = ia.cosine(lowerX, upperX)      
        # F_4 = F_3 / F_2 operation in the code list
        dfMin, dfMax = ia.divide(fMulMin, fMulMax, dfMin, dfMax)   
               
        # If the first order derivative with repect to any variable is 
        # undefined, the subregion should be kept for further inspection of 
        # first order derivatives with repect to other variables
        if (dfMin == -np.inf or dfMax == np.inf):
            flag = 0
        else:   
            # F_5 = sin (x_j) operation in the code list
            df2Min, df2Max = ia.sine(lowerX, upperX)   
            # F_6 = F_4 * F_5 operation in the code list
            dfMin, dfMax = ia.multiply(dfMin, dfMax, df2Min, df2Max)      
            # F_7 = x_j / n operation in the code list
            df2Min, df2Max = ia.times(lowerX, upperX, 1 / n) 
            # F_8 = F_6 + F_7 operation in the code list
            dfMin, dfMax = ia.add(dfMin, dfMax, df2Min, df2Max)      
...
def selectVarInSubRegion (xVal, num, pos, cycleIdx):
    ...
    "corruptedValue_IA", "corruptedSample_IA", and "corruptedDerivative_IA". 
...
def sampling (region, output_lowestUpper, numSamples, cycIdx):
    ...
    computes the smallest upper bound for the value of the Multimodal Corrupted 
    Convex Quadratic function among all sample points in the selected region to 
    update the upper bound of the global minimum of the Multimodal Corrupted 
    Convex Quadratic function. Specifically, each thread in the same block 
    computes the smallest upper bound for the value of the Multimodal Corrupted 
    Convex Quadratic function among all sample points in its corresponding 
    subregion and stores the smallest upper bound on an array on GPU shared 
    memory. The smallest element on the array is derived through a reduction 
    tree. The smallest element on the array is used to update the upper bound 
    of the global minimum of the Multimodal Corrupted Convex Quadratic function 
    (stored on GPU global memory) using atomic operations. 
    ...
    The upper bound of the global minimum of the Multimodal Corrupted Convex 
    Quadratic function. The value is initialized by the upper bound of the 
    global minimum of the Multimodal Corrupted Convex Quadratic function 
    ...
    value of the Multimodal Corrupted Convex Quadratic function derived by 
    ...
    # Compute the smallest upper bound for the value of the Multimodal Corrupted 
    # Convex Quadratic function among all sample points in a subregion 
    shr[thr] = corruptedSample_IA (region, pos, numSamples[0], cycIdx[0])
    ...
    minimum of the Multimodal Corrupted Convex Quadratic function using atomic 
    operations.
    '''
    if thr == 0:
...
def ruleOutGPU (region, result, atomic_index, lowestUpper, lowerBounds, cycIdx):
    ...
    could exist and the lower bounds for the value of the Multimodal Corrupted 
    Convex Quadratic function in these remaining subregions. The index of each 
    subregion is represented by the absolute position of the thread in the grid 
    in GPU computing. For the rule-out process, the kernel function first calls 
    the device function "corruptedValue_IA" which rules out suboptimal 
    subregions through computing the lower bound for the value of the function. 
    Then, the device function "corruptedDerivative_IA" is called to further 
    rule out suboptimal subregions through computing the first order 
    derivatives of the function. Specifically, the flag equals 0 if the 
    subregion is not ruled out. The flag equals 1 if the subregion is ruled out. 
    ...
    the global minimum of the Multimodal Corrupted Convex Quadratic function 
    ...
    The array stores the lower bounds for the value of the Multimodal Corrupted 
    Convex Quadratic function in the subregions that are not ruled out.
    ...
    fMin, flag = corruptedValue_IA (region,pos,lowestUpper[0],flag,cycIdx[0])
    # NOTE: Comment OUT the line if do NOT use derivative rule-out
    flag = corruptedDerivative_IA (region, pos, flag, cycIdx[0])
    if flag == 0:
...
def completeRegion (xVal, pos, cycleIdx):
    ...
    # the value of all variables X_i in the subregion in the Multimodal 
    # Corrupted Convex Quadratic function
...
# the 1000 variables in the Multimodal Corrupted Convex Quadratic function. 

initialRegion = np.tile([-10, 11], 1000)
...
# Initialize the maximum cycling index for the Multimodal Corrupted Convex 
# Quadratic function with 1000 variables
maximumCyc = 991
...
# to each region that may include the global minimum of the Multimodal Corrupted 
...
# the global minimum of the Multimodal Corrupted Convex Quadratic function F(X)
widthTolerance = 1e-4
...
# Print the number of variables for the Multimodal Corrupted Convex Quadratic 
# function, width tolerance and the number of samples for the sampling process
print("\n ---------------------------------------------------------------")
print("Number of variables for the Multimodal Corrupted Convex Quadratic function:", 1000)
...
while (terminate == False):
    ...
    # and update the upper bound of the global minimum of the Multimodal 
    # Corrupted Convex Quadratic function using the smallest upper bound for 
    # the value of the Multimodal Corrupted Convex Quadratic function among 
    # all sample points in the selected region
    ...
    # Transfer the upper bound of the global minimum of the Multimodal 
    # Corrupted Convex Quadratic function onto GPU global memory
    ...
    # the Multimodal Corrupted Convex Quadratic function 
    ...
    # Update the upper bound of the global minimum of the Multimodal 
    # Corrupted Convex Quadratic function 
    ...
    # possibly containing the global minimum of the Multimodal Corrupted Convex 
    # Quadratic function F(X))
    ...
    # value of the Multimodal Corrupted Convex Quadratic function in the 
    ...
    # bound of the global minimum of the Multimodal Corrupted Convex Quadratic 
    # function F(X) computed from the FIRST kernel function.
    ...
    # the values of the Multimodal Corrupted Convex Quadratic function from GPU 
    # to CPU
    ...
    # contain the global minimum of the Multimodal Corrupted Convex Quadratic 
    # function by appending the corresponding information of the subregions 
    # that are not ruled out (lower bounds for the value of the Multimodal 
    # Corrupted Convex Quadratic function in the current iteration, the index 
    # of the current iteration, the cycling index of the selected region, and 
    # the absolute positions of threads in the grid) after the CPU or GPU 
    # rule-out process.
    ...
    # The region with the smallest lower bound for the value of the Multimodal 
    # Corrupted Convex Quadratic function among all regions in the region list 
    # is selected and then removed from the region list. If the width in each 
    # dimension of the selected region is smaller than the user-defined width 
    # tolerance, the region is added to the output array as a region that may 
    # contain the global minimum. If the width is not smaller than the width 
    # tolerance, the region is selected as the region to be partitioned and 
    # analyzed in the next iteration step. The iterative process terminates 
    # when one of the two cases occurs: (1) the region selected for the next 
    ...
    if (lengthList != 0):
        while (found == False):
            ...
            # Obtain the lower bound for the value of the Multimodal Corrupted 
            # Convex Quadratic function in the selected region
            ...
            # Note: For the Multimodal Corrupted Convex Quadratic function with 
