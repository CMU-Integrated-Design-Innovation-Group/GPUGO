...
'''
Python script "griewank1000.py” finds the global minimum of the Griewank 
function using a GPU-based global optimization method. The Griewank function 
F(X) is first introduced by A. O. Griewank in 1981 [1]. The dimension of the 
Griewank function is set as 1000 in this script. In other words, the Griewank 
function has 1000 variables, and the value of each variable belongs to the range 
[-100, 110].
...
each subregion, and interval evaluation of the Griewank function is performed 
at each sample point. The smallest upper bound for the value of the Griewank 
...
Notably, for the Griewank function with more than 10 variables, a variable 
...
rounding in the module IAGriewank, the result of this script has 15 or fewer 
significant digits. 

Reference:
[1] Griewank, A. O., (1981) "Generalized Descent for Global Optimization". 
J. Optim. Theory Appl. 34(1), 11-39.
...
'''
================================================================================
Device functions: griewankValue_IA, griewankSample_IA, griewankDerivative_IA, 
and selectVarInSubRegion.
================================================================================
''' 

@cuda.jit(device=True)
def griewankValue_IA (xValIA, pos, minUpper, flag, cycleIdx):
    
    '''
    The device function "griewankValue_IA" is called by the kernel function 
    “ruleOutGPU”. The device function computes the lower and upper bounds for 
    the value of the Griewank function which determines if a subregion should 
    ...
    The upper bound of the global minimum of the Griewank function F(X).
    ...
    -------
    fMin: numpy.float64
    The lower bound for the value of the Griewank function F(X) in the 
    subregion.
    flag: int32
    The flag equals 0 if the subregion is not ruled out. The flag equals 1 if 
    the subregion is ruled out.
    ...
    n = 1000 # The number of variables in the Griewank function

    # Initialize the sum of terms within the multiplication sign of 
    # cos(X_i / sqrt(i))
    # F_3 = -F_1 operation in the code list
    fMin, fMax = -1, -1 

    # Compute the sum of terms within the multiplication sign of 
    # cos(X_i / sqrt(i))
    for i in range (1, n + 1):
        ...
        # F_m1 = (x_i) / (sqrt(i)) operation in the code list
        fsMinCur, fsMaxCur = ia.times(lowerX, upperX, 1 / math.sqrt(i)) 
        # F_m2 = cosine (F_m1) operation in the code list
    ...
    # F_4 = (1/4000) * F_2 operation in the code list
    fsMin, fsMax = ia.times(fsMin, fsMax, 1 / 4000)
    # F_5 = 1 + F_4 operation in the code list
    fsMin, fsMax = ia.plus(fsMin, fsMax, 1)
...
@cuda.jit(device=True)
def griewankSample_IA (xValIA, pos, num, cycleIdx):
    
    '''
    The device function "griewankSample_IA" is called by the kernel function 
    “sampling”. The device function "griewankSample_IA" samples over a 
    subregion and derives the smallest upper bound for the value of the 
    Griewank function among all sample points in the subregion. Specifically, 
    ...
    the Griewank function at each sample point are then computed using 
    interval arithmetic. The smallest upper bound for the value of the 
    Griewank function among all sample points is returned as the result. 
    ...
    -------
    fMax: numpy.float64
    The smallest upper bound for the value of the Griewank function among all 
    ...
    for j in range (0, num):
        ...
        for i in range (1, n + 1):
            ...
            # F_m1 = (x_i) / (sqrt(i)) operation in the code list
            fsMinCur, fsMaxCur = ia.times(stepCurr, stepCurr, 1 / math.sqrt(i)) 
            # F_m2 = cosine (F_m1) operation in the code list
            fsMinCur, fsMaxCur = ia.cosine(fsMinCur, fsMaxCur)  
            # F_1 = multiply(F_m2) operation in the code list
            fMin, fMax = ia.multiply(fMin, fMax, fsMinCur, fsMaxCur)    
        ...
        # value of the Griewank function among all sample points within the 
        # subregion
        if (j == 0):
            ...
            # the value of the Griewank function among all sample points in the
            # subregion
            if (fMaxOut > fMax):
                fMaxOut = fMax
...
@cuda.jit(device=True)
def griewankDerivative_IA (xdIA, pos, flagVal, cycleIdx):
    
    '''
    Device function "griewankDerivative_IA" is called by the kernel function 
    ...
    for i in range (1, n + 1):
        ...
        fsMinCur, fsMaxCur = ia.cosine(fsMinCur, fsMaxCur)  
        # F_1 = multiply(F_m2) operation in the code list
        fMulMin, fMulMax = ia.multiply(fMulMin, fMulMax, fsMinCur, fsMaxCur)   
    
    # Calculate the first order derivatives of the Griewank Function            
    for j in range (1, n + 1):
        lowerX, upperX = selectVarInSubRegion (xdIA, j, pos, cycleIdx)  
        # F_2 = x_j / sqrt(j) operation in the code list
        df2Min, df2Max = ia.times(lowerX, upperX, 1 / math.sqrt(j)) 
        # F_3 = cos (F_2) operation in the code list
        dfMin, dfMax = ia.cosine(df2Min, df2Max) 
        ...
        if (dfMin == -np.inf or dfMax == np.inf):
            ...
            # F_4 = F_1 / F_3 operation in the code list
            dfMin, dfMax = ia.divide(fMulMin, fMulMax, dfMin, dfMax)     
            # F_5 = sin (F_2) operation in the code list
            df2Min, df2Max = ia.sine(df2Min, df2Max)   
            # F_6 = F_4 * F_5 operation in the code list
            dfMin, dfMax = ia.multiply(dfMin, dfMax, df2Min, df2Max)     
            # F_7 = (1 / sqrt(j)) * F_6 operation in the code list
            dfMin, dfMax = ia.times(dfMin, dfMax, 1 / math.sqrt(j))   
            # F_8 = x_j / 2000 operation in the code list
            df2Min, df2Max = ia.times(lowerX, upperX, 1 / 2000)   
            # F_9 = F_8 + F_7 operation in the code list
...
def selectVarInSubRegion (xVal, num, pos, cycleIdx):
    ...
    "griewankValue_IA", "griewankSample_IA", and "griewankDerivative_IA". 
    ...
    length = 1000 # The number of variables in the Griewank function
...
def sampling (region, output_lowestUpper, numSamples, cycIdx):
    ...
    computes the smallest upper bound for the value of the Griewank function 
    among all sample points in the selected region to update the upper bound of 
    the global minimum of the Griewank function. Specifically, each thread in 
    the same block computes the smallest upper bound for the value of the 
    Griewank function among all sample points in its corresponding subregion 
    ...
    global minimum of the Griewank function (stored on GPU global memory) 
    ...
    The upper bound of the global minimum of the Griewank function. The value 
    is initialized by the upper bound of the global minimum of the Griewank 
    ...
    value of the Griewank function derived by threads in the same block. The 
    ...
    # Compute the smallest upper bound for the value of the Griewank function 
    # among all sample points in a subregion 
    shr[thr] = griewankSample_IA (region, pos, numSamples[0], cycIdx[0])
    ...
    minimum of the Griewank function using atomic operations.
    '''
    if thr == 0:
...
def ruleOutGPU (region, result, atomic_index, lowestUpper, lowerBounds, cycIdx):
    ...
    could exist and the lower bounds for the value of the Griewank function in 
    ...
    "griewankValue_IA" which rules out suboptimal subregions through computing 
    the lower bound for the value of the function. Then, the device function 
    "griewankDerivative_IA" is called to further rule out suboptimal subregions 
    ...
    the global minimum of the Griewank function F(X). This array is initialized 
    ...
    The array stores the lower bounds for the value of the Griewank function in 
    ...
    fMin, flag = griewankValue_IA (region,pos,lowestUpper[0],flag,cycIdx[0])
    # NOTE: Comment OUT the line if do NOT use derivative rule-out
    flag = griewankDerivative_IA (region, pos, flag, cycIdx[0])
    if flag == 0:
...
def completeRegion (xVal, pos, cycleIdx):
    ...
    # the value of all variables X_i in the subregion in the Griewank function
...
# the 1000 variables in the Griewank function. 

initialRegion = np.tile([-100, 110], 1000)
...
# Initialize the maximum cycling index for the Griewank function with 1000 
# variables
maximumCyc = 991
...
# to each region that may include the global minimum of the Griewank function)
...
# the global minimum of the Griewank function F(X)
widthTolerance = 1e-4
...
# Print the number of variables for the Griewank function, width tolerance and 
# the number of samples for the sampling process
print("\n ---------------------------------------------------------------")
print("Number of variables for the Griewank function:", 1000)
...
while (terminate == False):
    ...
    # and update the upper bound of the global minimum of the Griewank 
    # function using the smallest upper bound for the value of the Griewank 
    ...
    # Transfer the upper bound of the global minimum of the Griewank function 
    ...
    # Update the upper bound of the global minimum of the Griewank function 
    ...
    # possibly containing the global minimum of the Griewank function F(X))
    ...
    # value of the Griewank function in the subregions that could contain the 
    # global minimum 
    ...
    # bound of the global minimum of the Griewank function F(X) computed from 
    ...
    # the values of the Griewank function from GPU to CPU
    ...
    # contain the global minimum of the Griewank function by appending the 
    # corresponding information of the subregions that are not ruled out (lower 
    # bounds for the value of the Griewank function in the current iteration, 
    ...
    # The region with the smallest lower bound for the value of the Griewank 
    ...
    if (lengthList != 0):
        while (found == False):
            ...
            # Obtain the lower bound for the value of the Griewank function in 
            # the selected region
            ...
            # Note: For the Griewank function with MORE THAN 10 variables, the 
