...
'''
Python script "auckley1000.py” finds the global minimum of the Ackley function 
using a GPU-based global optimization method. The Ackley function F(X) is 
first introduced in the book A Connectionist Machine for Genetic Programming by 
David H. Ackley [1] in 1987. The function utilized in the script is adapted 
from the Equation (26) in the paper written by Bäck and Schwefel [2] which 
provides the generalized form of the function. The dimension of the Ackley 
function is set as 1000 in this script. In other words, the Ackley function has 
1000 variables, and the value of each variable belongs to the range [-35, 40]. 
...
each subregion, and interval evaluation of the Ackley function is performed at 
each sample point. The smallest upper bound for the value of the Ackley function 
...
Notably, for the Ackley function with more than 10 variables, a variable cycling 
technique is employed. Specifically, when the selected region is partitioned 
into many subregions in Step 2 and Step 3, only 10 dimensions of the selected 
region is partitioned in turn at each iteration, instead of partitioning all 
its dimensions at the same time. 
...
rounding in the module IAAckley, the result of this script has 15 or fewer 
significant digits. 

Reference:
[1] David H. Ackley, (1987) A Connectionist Machine for Genetic Hillclimbing. 
Kluwer Academic Publishers, USA.
https://doi.org/10.1007/978-1-4613-1997-9

[2] Bäck, Thomas & Schwefel, Hans-Paul, (1993) "An Overview of Evolutionary 
Algorithms for Parameter Optimization". Evolutionary Computation. 1: 1-23.
https://doi.org/10.1162/evco.1993.1.1.1

'''

from numba import cuda
import numpy as np
import IAAckley as ia
import math
import time
...
'''
================================================================================
Device functions: ackleyValue_IA, ackleySample_IA, ackleyDerivative_IA, and 
selectVarInSubRegion.
================================================================================
''' 

@cuda.jit(device=True)
def ackleyValue_IA (xValIA, pos, minUpper, flag, cycleIdx):
    
    '''
    The device function "ackleyValue_IA" is called by the kernel function 
    “ruleOutGPU”. The device function computes the lower and upper bounds for 
    the value of the Ackley function which determines if a subregion should be 
    ruled out using a code list. A code list defines the sequence of interval 
    ...
    in the function F(X), where i = 1, 2, … n. In this script, n = d = 1000.

    pos: int32
    The absolute position of a thread in the grid in GPU computing.

    minUpper: numpy.float64
    The upper bound of the global minimum of the Ackley function F(X).
    ...
    -------
    fMin: numpy.float64
    The lower bound for the value of the Ackley function F(X) in the subregion.
    flag: int32
    The flag equals 0 if the subregion is not ruled out. The flag equals 1 if 
    the subregion is ruled out.
    ...
    d = 1000 # The number of variables in the Ackley function

    # Interval evaluation of the Ackley function in the subregion
    ...
    for i in range (1, d + 1):
        ...
        # F_1 = sum (F_s1) operation in the code list
        fsMin, fsMax = ia.add(fsMin, fsMax, fsMinCur, fsMaxCur)
    # F_2 = d ^ (-1) * F_1 operation in the code list
    fMin, fMax = ia.times(fsMin, fsMax, 1 / d)
    # F_3 = (F_2) ^ (1 / 2) operation in the code list 
    fMin, fMax = ia.power(fMin, fMax, 1 / 2)   
    # F_4 = -0.02 * F_1 operation in the code list
    fMin, fMax = ia.times(fMin, fMax, -0.02)
    # F_5 = e ^ F_4 operation in the code list
    fMin, fMax = ia.exp(fMin, fMax)
    # F_6 = -20 * F_5 operation in the code list
    fMin, fMax = ia.times(fMin, fMax, -20) 
    
    # Initialize the sum of terms within the summation sign of cos(2pi * x_i)
    fsMin, fsMax = 0, 0 
    # Compute the sum of terms within the summation sign of cos(2pi * x_i)
    for i in range (1, d + 1):
        # Update the lower and upper bounds for the values of variables X_i in 
        # each iteration
        lowerX, upperX = selectVarInSubRegion (xValIA, i, pos, cycleIdx)
        # F_s2 = 2pi * x_i operation in the code list
        fsMinCur, fsMaxCur = ia.times(lowerX, upperX, 2 * math.pi)
        # F_s3 = cos (F_s2) operation in the code list
        fsMinCur, fsMaxCur = ia.cosine(fsMinCur, fsMaxCur)
        # F_7 = sum (F_s3) operation in the code list
        fsMin, fsMax = ia.add(fsMin, fsMax, fsMinCur, fsMaxCur)   
    # F_8 = d ^ (-1) * F_7 operation in the code list
    f8Min, f8Max = ia.times(fsMin, fsMax, 1 / d)      
    # F_9 = e ^ F_8 operation in the code list
    f8Min, f8Max = ia.exp(f8Min, f8Max)
    # F_10 = F_6 - F_9 operation in the code list
    fMin, fMax = ia.minus(fMin, fMax, f8Min, f8Max) 
    # F_11 = F_10 + 20 + e operation in the code list
    fMin, fMax = ia.plus(fMin, fMax, 20 + math.exp(1))    
...
@cuda.jit(device=True)
def ackleySample_IA (xValIA, pos, num, cycleIdx):
    
    '''
    The device function "ackleySample_IA" is called by the kernel function 
    “sampling”. The device function "ackleySample_IA" samples over a subregion 
    and derives the smallest upper bound for the value of the Ackley function 
    among all sample points in the subregion. Specifically, the bounds for the 
    value of each variable within the subregion (the position of the subregion) 
    are first calculated using the absolute position of the thread in the grid 
    that corresponds to the subregion, the bounds for the value of each variable 
    within the selected region (the position of the selected region), and the 
    cycling index of the selected region. User-specified number of sample points 
    are uniformly chosen along the diagonal of the subregion, where the diagonal 
    connects the two vertices of the subregion with the coordinates of lower 
    bounds and upper bounds for all variables, respectively. The lower and upper 
    bounds for the value of the Ackley function at each sample point are then 
    computed using interval arithmetic. The smallest upper bound for the value 
    of the Ackley function among all sample points is returned as the result. 
    ...
    function F(X), where i belongs to {1, 2, …, n}. In this script, 
    n = d = 1000.
    ...
    -------
    fMax: numpy.float64
    The smallest upper bound for the value of the Ackley function among all 
    ...
    for j in range (0, num):
        ...
        # Compute the sum of terms within the summation sign of (x_i) ^ 2
        for i in range (1, d + 1):
            # Initialize the lower and upper bounds for the variables X_1 and 
            # X_n
        ...
        for i in range (1, d + 1):
            ...
            # F_s2 = 2pi * x_i operation in the code list
            fsMinCur, fsMaxCur = ia.times(stepCurr, stepCurr, 2 * math.pi)
            # F_s3 = cos (F_s2) operation in the code list
            fsMinCur, fsMaxCur = ia.cosine(fsMinCur, fsMaxCur)
            # F_7 = sum (F_s3) operation in the code list
            fsMin, fsMax = ia.add(fsMin, fsMax, fsMinCur, fsMaxCur)   
        ...
        # value of the Ackley function among all sample points within the 
        # subregion
        if (j == 0):
            ...
            # the value of the Ackley function among all sample points in the
            # subregion
            if (fMaxOut > fMax):
                fMaxOut = fMax
...
@cuda.jit(device=True)
def ackleyDerivative_IA (xdIA, pos, flagVal, cycleIdx):
    
    '''
    Device function "ackleyDerivative_IA" is called by the kernel function 
    ...
    # Calculate first constant term F_7
    # Initialize the sum of terms within the summation sign of (x_i) ^ 2
    dfsMin, dfsMax = 0, 0 
    # Compute the sum of terms within the summation sign of (x_i) ^ 2
    for i in range (1, d + 1):
        ...
        # F_s1 = (x_i) ^ 2 operation in the code list
        dfsMinCur, dfsMaxCur = ia.power(lowerX, upperX, 2)
        # F_1 = sum (F_s1) operation in the code list
        dfsMin, dfsMax = ia.add(dfsMin, dfsMax, dfsMinCur, dfsMaxCur)
    # F_2 = d ^ (-1) * F_1 operation in the code list
    dfMin, dfMax = ia.times(dfsMin, dfsMax, 1 / d)
    # F_3 = (F_2) ^ (1 / 2) operation in the code list 
    df3Min, df3Max = ia.power(dfMin, dfMax, 1 / 2)   
    # F_4 = -0.02 * F_1 operation in the code list
    dfMin, dfMax = ia.times(df3Min, df3Max, -0.02)
    # F_5 = e ^ F_4 operation in the code list
    dfMin, dfMax = ia.exp(dfMin, dfMax)
    # F_6 = F_5 / F_3 operation in the code list
    dfMin, dfMax = ia.divide(dfMin, dfMax, df3Min, df3Max)  
    # F_7 = 0.4 * F_6 operation in the code list
    dfMin, dfMax = ia.times(dfMin, dfMax, 0.4) 

    # Calculate Second constant term F_12
    # Initialize the sum of terms within the summation sign of cos(2pi * x_i)
    dfsMin, dfsMax = 0, 0 
    # Compute the sum of terms within the summation sign of cos(2pi * x_i)
    for i in range (1, d + 1):
        ...
        lowerX, upperX = selectVarInSubRegion (xdIA, i, pos, cycleIdx)
        # F_s2 = 2pi * x_i operation in the code list
        dfsMinCur, dfsMaxCur = ia.times(lowerX, upperX, 2 * math.pi)
        # F_s3 = cos (F_s2) operation in the code list
        dfsMinCur, dfsMaxCur = ia.cosine(dfsMinCur, dfsMaxCur)
        # F_9 = sum (F_s3) operation in the code list
        dfsMin, dfsMax = ia.add(dfsMin, dfsMax, dfsMinCur, dfsMaxCur)      
    # F_10 = d ^ (-1) * F_9 operation in the code list
    df10Min, df10Max = ia.times(dfsMin, dfsMax, 1 / d)        
    # F_11 = e ^ F_10 operation in the code list
    df10Min, df10Max = ia.exp(df10Min, df10Max)
    # F_12 = 2pi * d ^ (-1) * F_11 operation in the code list
    df10Min, df10Max = ia.times(df10Min, df10Max, 2 * math.pi * (1 / d)) 
    
    # Calculate the first order derivatives of the Ackley Function            
    for j in range (1, d + 1):
        lowerX, upperX = selectVarInSubRegion (xdIA, j, pos, cycleIdx)
        # F_8 = F_7 * x_j operation in the code list
        dfMin, dfMax = ia.multiply(dfMin, dfMax, lowerX, upperX)             
        # F_13 = 2pi * x_j operation in the code list
        df13Min, df13Max = ia.times(lowerX, upperX, 2 * math.pi)       
        # F_14 = sin (F_13) operation in the code list
        df13Min, df13Max = ia.sine(df13Min, df13Max)    
        # F_15 = F_12 * F_14 operation in the code list
        df13Min, df13Max = ia.multiply(df10Min, df10Max, df13Min, df13Max)  
        # F_16 = F_8 + F_15 operation in the code list
        dfMin, dfMax = ia.add(dfMin, dfMax, df13Min, df13Max)    

        # Check the derivative to determine if the subregion could be ruled out
        if (dfMin > 0 and lowerX != -35):
            flag = 1
            return flag
        elif (dfMax < 0 and upperX != 40):
            flag = 1
            return flag
...
def selectVarInSubRegion (xVal, num, pos, cycleIdx):
    ...
    "ackleyValue_IA", "ackleySample_IA", and "ackleyDerivative_IA". The device 
    ...
    length = 1000 # The number of variables in the Ackley function
...
def sampling (region, output_lowestUpper, numSamples, cycIdx):
    ...
    computes the smallest upper bound for the value of the Ackley function among 
    all sample points in the selected region to update the upper bound of the 
    global minimum of the Ackley function. Specifically, each thread in the same 
    block computes the smallest upper bound for the value of the Ackley function 
    ...
    Ackley function (stored on GPU global memory) using atomic operations. 
    ...
    The upper bound of the global minimum of the Ackley function. The value is 
    initialized by the upper bound of the global minimum of the Ackley function 
    ...
    value of the Ackley function derived by threads in the same block. The size 
    ...
    # Compute the smallest upper bound for the value of the Ackley function 
    # among all sample points in a subregion 
    shr[thr] = ackleySample_IA (region, pos, numSamples[0], cycIdx[0])
    ...
    minimum of the Ackley function using atomic operations.
    '''
    if thr == 0:
...
def ruleOutGPU (region, result, atomic_index, lowestUpper, lowerBounds, cycIdx):
    ...
    could exist and the lower bounds for the value of the Ackley function in 
    ...
    "ackleyValue_IA" which rules out suboptimal subregions through computing the 
    lower bound for the value of the function. Then, the device function 
    "ackleyDerivative_IA" is called to further rule out suboptimal subregions 
    ...
    the global minimum of the Ackley function F(X). This array is initialized 
    ...
    The array stores the lower bounds for the value of the Ackley function in 
    ...
    fMin, flag = ackleyValue_IA (region,pos,lowestUpper[0],flag,cycIdx[0])
    # NOTE: Comment OUT the line if do NOT use derivative rule-out
    flag = ackleyDerivative_IA (region, pos, flag, cycIdx[0])
    if flag == 0:
...
def completeRegion (xVal, pos, cycleIdx):
    ...
    # the value of all variables X_i in the subregion in the Ackley function
...
# the 1000 variables in the Ackley function. 

initialRegion = np.tile([-35, 40], 1000)
...
# Initialize the maximum cycling index for the Ackley function with 1000 
# variables
maximumCyc = 991
...
# to each region that may include the global minimum of the Ackley function)
cycling_list = np.array([], dtype = np.int32)

# Initialize the initial smallest upper bound for the global minimum as 1000
lowestUpperVal = 1000
...
# the global minimum of the Ackley function F(X)
widthTolerance = 1e-4
...
# Print the number of variables for the Ackley function, width tolerance and 
# the number of samples for the sampling process
print("\n ---------------------------------------------------------------")
print("Number of variables for the Ackley function:", 1000)
...
while (terminate == False):
    ...
    # and update the upper bound of the global minimum of the Ackley 
    # function using the smallest upper bound for the value of the Ackley 
    ...
    # Transfer the upper bound of the global minimum of the Ackley function 
    ...
    # Update the upper bound of the global minimum of the Ackley function 
    ...
    # possibly containing the global minimum of the Ackley function F(X))
    ...
    # value of the Ackley function in the subregions that could contain the 
    # global minimum 
    ...
    # bound of the global minimum of the Ackley function F(X) computed from the 
    ...
    # the values of the Ackley function from GPU to CPU
    ...
    # contain the global minimum of the Ackley function by appending the 
    # corresponding information of the subregions that are not ruled out (lower 
    # bounds for the value of the Ackley function in the current iteration, the 
    ...
    # The region with the smallest lower bound for the value of the Ackley 
    ...
    if (lengthList != 0):
        while (found == False):
            ...
            # Obtain the lower bound for the value of the Ackley function in the 
            # selected region
            ...
            # Note: For the Ackley function with MORE THAN 10 variables, the 
