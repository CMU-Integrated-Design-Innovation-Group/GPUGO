...
'''
Python script "rastrigin1000.py” finds the global minimum of the Rastrigin 
function using a GPU-based global optimization method. The Rastrigin function 
F(X) is first introduced in the work Extremal Control Systems, Theoretical 
Foundations of Engineering Cybernetics Series in 1974 [1]. The dimension of the 
Rastrigin function is set as 1000 in this script. In other words, the Rastrigin 
...
each subregion, and interval evaluation of the Rastrigin function is performed 
at each sample point. The smallest upper bound for the value of the Rastrigin 
function among all sample points is used to update the upper bound of the 
global minimum of the Rastrigin function.
...
Notably, for the Rastrigin function with more than 10 variables, a variable 
...
When the iterative process is halted, the 1000-dimensional region(s) that satisfy 
...
rounding in the module IARastrigin, the result of this script has 15 or fewer 
significant digits. 

Reference:
[1] Rastrigin, L. A., (1974) Extremal Control Systems. Theoretical Foundations 
of Engineering Cybernetics Series. 3.

'''

from numba import cuda
import numpy as np
import IARastrigin as ia
import math
import time
...
'''
================================================================================
Device functions: rastriginValue_IA, rastriginSample_IA, rastriginDerivative_IA, 
and selectVarInSubRegion.
================================================================================
''' 

@cuda.jit(device=True)
def rastriginValue_IA (xValIA, pos, minUpper, flag, cycleIdx):
    
    '''
    The device function "rastriginValue_IA" is called by the kernel function 
    “ruleOutGPU”. The device function computes the lower and upper bounds for 
    the value of the Rastrigin function which determines if a subregion should 
    ...
    The upper bound of the global minimum of the Rastrigin function F(X).
    ...
    -------
    fMin: numpy.float64
    The lower bound for the value of the Rastrigin function F(X) in the 
    subregion.
    flag: int32
    The flag equals 0 if the subregion is not ruled out. The flag equals 1 if 
    the subregion is ruled out.
    ...
    n = 1000 # The number of variables in the Rastrigin function

    # Interval evaluation of the Rastrigin function in the subregion
    ...
    for i in range (1, n + 1):
        ...
        # F_s2 = 2 \pi x_i operation in the code list
        fs2Min, fs2Max = ia.times(lowerX, upperX, 2 * math.pi)
        # F_s3 = cos (F_s2) operation in the code list
        fs2Min, fs2Max = ia.cosine(fs2Min, fs2Max)
        # F_s4 = 10 (F_s3) operation in the code list
        fs2Min, fs2Max = ia.times(fs2Min, fs2Max, 10)     
        # F_s5 = F_s1 - F_s4 operation in the code list
        fsMinCur, fsMaxCur = ia.minus(fsMinCur, fsMaxCur, fs2Min, fs2Max)  
        # F_1 = sum (F_s5) operation in the code list
        fsMin, fsMax = ia.add(fsMin, fsMax, fsMinCur, fsMaxCur)  
    # F_2 = 10n + F_1 operation in the code list
    fMin, fMax = ia.plus(fsMin, fsMax, 10 * n)     
...
@cuda.jit(device=True)
def rastriginSample_IA (xValIA, pos, num, cycleIdx):
    
    '''
    The device function "rastriginSample_IA" is called by the kernel function 
    “sampling”. The device function "rastriginSample_IA" samples over a 
    subregion and derives the smallest upper bound for the value of the 
    Rastrigin function among all sample points in the subregion. Specifically, 
    ...
    the Rastrigin function at each sample point are then computed using 
    interval arithmetic. The smallest upper bound for the value of the 
    Rastrigin function among all sample points is returned as the result. 
    ...
    -------
    fMax: numpy.float64
    The smallest upper bound for the value of the Rastrigin function among all 
    ...
    for j in range (0, num):
        ...
        for i in range (1, n + 1):
            ...
            # F_s2 = 2 \pi x_i operation in the code list
            fs2Min, fs2Max = ia.times(stepCurr, stepCurr, 2 * math.pi)
            # F_s3 = cos (F_s2) operation in the code list
            fs2Min, fs2Max = ia.cosine(fs2Min, fs2Max)
            # F_s4 = 10 (F_s3) operation in the code list
            fs2Min, fs2Max = ia.times(fs2Min, fs2Max, 10)     
        ...
        # value of the Rastrigin function among all sample points within the 
        # subregion
        if (j == 0):
            ...
            # the value of the Rastrigin function among all sample points in the
            # subregion
            if (fMaxOut > fMax):
                fMaxOut = fMax
...
@cuda.jit(device=True)
def rastriginDerivative_IA (xdIA, pos, flagVal, cycleIdx):
    
    '''
    Device function "rastriginDerivative_IA" is called by the kernel function 
    ...
    # Calculate the first order derivatives of the Rastrigin Function            
    for j in range (1, n + 1):
        lowerX, upperX = selectVarInSubRegion (xdIA, j, pos, cycleIdx)
        
        # F_1 = 2 x_i operation in the code list
        dfMin, dfMax = ia.times(lowerX, upperX, 2)
        # F_2 = 2 \pi x_i operation in the code list
        df2Min, df2Max = ia.times(lowerX, upperX, 2 * math.pi)    
        # F_3 = sin (F_2) operation in the code list
        df2Min, df2Max = ia.sine(df2Min, df2Max)    
        # F_4 = 20 \pi F_3 operation in the code list
        df2Min, df2Max = ia.times(df2Min, df2Max, 20 * math.pi)  
        ...
        if (dfMin > 0 and lowerX != -5.5):
            flag = 1
            return flag
        elif (dfMax < 0 and upperX != 6):
            flag = 1
            return flag
...
def selectVarInSubRegion (xVal, num, pos, cycleIdx):
    ...
    "rastriginValue_IA", "rastriginSample_IA", and "rastriginDerivative_IA". 
    ...
    length = 1000 # The number of variables in the Rastrigin function
...
def sampling (region, output_lowestUpper, numSamples, cycIdx):
    ...
    computes the smallest upper bound for the value of the Rastrigin function 
    among all sample points in the selected region to update the upper bound of 
    the global minimum of the Rastrigin function. Specifically, each thread in 
    the same block computes the smallest upper bound for the value of the 
    Rastrigin function among all sample points in its corresponding subregion 
    ...
    global minimum of the Rastrigin function (stored on GPU global memory) 
    ...
    The upper bound of the global minimum of the Rastrigin function. The value 
    is initialized by the upper bound of the global minimum of the Rastrigin 
    ...
    value of the Rastrigin function derived by threads in the same block. The 
    ...
    # Compute the smallest upper bound for the value of the Rastrigin function 
    # among all sample points in a subregion 
    shr[thr] = rastriginSample_IA (region, pos, numSamples[0], cycIdx[0])
    ...
    minimum of the Rastrigin function using atomic operations.
    '''
    if thr == 0:
...
def ruleOutGPU (region, result, atomic_index, lowestUpper, lowerBounds, cycIdx):
    ...
    could exist and the lower bounds for the value of the Rastrigin function in 
    ...
    "rastriginValue_IA" which rules out suboptimal subregions through computing 
    the lower bound for the value of the function. Then, the device function 
    "rastriginDerivative_IA" is called to further rule out suboptimal subregions 
    ...
    the global minimum of the Rastrigin function F(X). This array is initialized 
    ...
    The array stores the lower bounds for the value of the Rastrigin function in 
    ...
    fMin, flag = rastriginValue_IA (region,pos,lowestUpper[0],flag,cycIdx[0])
    # NOTE: Comment OUT the line if do NOT use derivative rule-out
    flag = rastriginDerivative_IA (region, pos, flag, cycIdx[0])
    if flag == 0:
...
def completeRegion (xVal, pos, cycleIdx):
    ...
    # the value of all variables X_i in the subregion in the Rastrigin function
...
# the 1000 variables in the Rastrigin function. 

initialRegion = np.tile([-5.5, 6], 1000)
...
# Initialize the maximum cycling index for the Rastrigin function with 1000 
# variables
maximumCyc = 991
...
# to each region that may include the global minimum of the Rastrigin function)
...
# the global minimum of the Rastrigin function F(X)
widthTolerance = 1e-4
...
# Print the number of variables for the Rastrigin function, width tolerance and 
# the number of samples for the sampling process
print("\n ---------------------------------------------------------------")
print("Number of variables for the Rastrigin function:", 1000)
...
while (terminate == False):
    ...
    # and update the upper bound of the global minimum of the Rastrigin 
    # function using the smallest upper bound for the value of the Rastrigin 
    ...
    # Transfer the upper bound of the global minimum of the Rastrigin function 
    ...
    # Update the upper bound of the global minimum of the Rastrigin function 
    ...
    # possibly containing the global minimum of the Rastrigin function F(X))
    ...
    # value of the Rastrigin function in the subregions that could contain the 
    # global minimum 
    ...
    # bound of the global minimum of the Rastrigin function F(X) computed from 
    ...
    # the values of the Rastrigin function from GPU to CPU
    ...
    # contain the global minimum of the Rastrigin function by appending the 
    # corresponding information of the subregions that are not ruled out (lower 
    # bounds for the value of the Rastrigin function in the current iteration, 
    ...
    # The region with the smallest lower bound for the value of the Rastrigin 
    ...
    if (lengthList != 0):
        while (found == False):
            ...
            # Obtain the lower bound for the value of the Rastrigin function in 
            # the selected region
            ...
            # Note: For the Rastrigin function with MORE THAN 10 variables, the 
